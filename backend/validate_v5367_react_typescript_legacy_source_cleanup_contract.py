...
checks = {
    ...
    'launcher version': '$FallbackAgentStudioVersion = "5.368"' in PS1,
...
if failed:
    raise SystemExit('FAIL v5.368: ' + ', '.join(failed))
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    if not result.get('ok') or len(result.get('removed') or []) != 3:
        raise SystemExit(f'FAIL v5.368 functional cleanup result: {result!r}')
    if any(path.exists() for path in legacy):
        raise SystemExit('FAIL v5.368 functional cleanup: legacy file still exists')
    if not all(row.get('deleted') and row.get('verified') for row in result.get('patch_rows') or []):
        raise SystemExit('FAIL v5.368 functional cleanup: patch row not verified/deleted')
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    if not path.exists() or result.get('removed'):
        raise SystemExit('FAIL v5.368 non-TypeScript project must preserve App.jsx')

print('PASS v5.368 React TypeScript Legacy Source Cleanup contract')
