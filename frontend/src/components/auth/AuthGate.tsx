...
async function withTimeout<T>(promise:Promise<T>,ms: LegacyValue=AUTH_BOOT_TIMEOUT_MS):Promise<T>{
  let timer=0
  try{return await Promise.race([promise,new Promise<T>((_: LegacyValue,reject: LegacyValue)=>{timer=window.setTimeout(()=>reject(new Error(`로그인 상태 확인이 ${Math.round(ms/1000)}초 안에 완료되지 않았습니다.`)),ms)})])}
  finally{if(timer)window.clearTimeout(timer)}
}
const delay=(ms:number)=>new Promise<void>((resolve: LegacyValue)=>window.setTimeout(resolve,ms))
...
export function AuthGate({children}:{children:ReactNode}){
  ...
  const loadPcStatus=async()=>{
    const result=await withTimeout(api<LegacyValue>('/auth/current-pc'))
  ...
  useEffect(()=>{
    ...
    const boot=async()=>{
      ...
      for(let attempt=0;attempt<AUTH_BOOT_RETRY_DELAYS_MS.length;attempt++){
        ...
        if(disposed)return
        try{
          const r=await withTimeout(api<LegacyValue>('/auth/me'))
          if(disposed)return
          setMember(r.member)
  ...
  useEffect(()=>{
    const sync=(event:StorageEvent)=>{
      if(event.key==='theanova.agentstudio.auth.token')setAuthRetryNonce((value: LegacyValue)=>value+1)
    }
  ...
  useEffect(()=>{
    ...
    const openMenu=(event:Event)=>{
      ...
      setMenuOpen((prev: LegacyValue)=>{const next=!prev;if(next)void loadPcStatus().catch((err: LegacyValue)=>setMessage(String(err)));return next})
    }
  ...
  const login=async(e:FormEvent<HTMLFormElement>)=>{
    ...
    try{
      const r=await withTimeout(api<LegacyValue>('/auth/login',{method:'POST',body:JSON.stringify({login_id:fd.get('login_id'),password:fd.get('password'),remember_me:remember})}),15000)
...
function MemberAdmin({onClose}:{onClose:()=>void}){
  const [items,setItems]=useState<Member[]>([]);const [pcs,setPcs]=useState<PcRow[]>([]);const [message,setMessage]=useState('')
  const load=async()=>{try{const [m,p]=await Promise.all([api<LegacyValue>('/auth/members'),api<LegacyValue>('/auth/pcs')]);setItems(m.items||[]);setPcs(p.items||[])}catch(e){setMessage(String(e))}}
  useEffect(()=>{load()},[])
  const togglePc=(index:number,pcName:string,checked:boolean)=>setItems((v: LegacyValue)=>v.map((x: LegacyValue,n: LegacyValue)=>n===index?{...x,pcs:checked?Array.from(new Set([...(x.pcs||[]),pcName])):(x.pcs||[]).filter((p: LegacyValue)=>p!==pcName)}:x))
  ...
  return <section className="member-admin-page"><div className="member-admin"><header><div><strong>회원 관리</strong><small>회원 정보·권한·사용 가능 PC를 관리합니다. 한 사용자가 집 PC, 학원 노트북 등 여러 PC를 동시에 등록할 수 있습니다.</small></div><button onClick={onClose}>본문으로 돌아가기</button></header>{message&&<div className="auth-message">{message}</div>}<div className="member-table"><table><thead><tr><th>아이디</th><th>이름</th><th>이메일</th><th>권한</th><th>사용</th><th>관리 PC</th><th></th></tr></thead><tbody>{items.map((row: LegacyValue,i: LegacyValue)=><tr key={row.id}><td>{row.login_id}</td><td><input value={row.name} onChange={(e: LegacyValue)=>setItems((v: LegacyValue)=>v.map((x: LegacyValue,n: LegacyValue)=>n===i?{...x,name:e.target.value}:x))}/></td><td><input value={row.email} onChange={(e: LegacyValue)=>setItems((v: LegacyValue)=>v.map((x: LegacyValue,n: LegacyValue)=>n===i?{...x,email:e.target.value}:x))}/></td><td><select value={row.role} onChange={(e: LegacyValue)=>setItems((v: LegacyValue)=>v.map((x: LegacyValue,n: LegacyValue)=>n===i?{...x,role:e.target.value}:x))}><option>USER</option><option>ADMIN</option></select></td><td><input type="checkbox" checked={row.is_active} onChange={(e: LegacyValue)=>setItems((v: LegacyValue)=>v.map((x: LegacyValue,n: LegacyValue)=>n===i?{...x,is_active:e.target.checked}:x))}/></td><td><div className="member-pc-list">{pcs.length?pcs.map((pc: LegacyValue)=><label key={pc.pc_name}><input type="checkbox" checked={(row.pcs||[]).includes(pc.pc_name)} onChange={(e: LegacyValue)=>togglePc(i,pc.pc_name,e.target.checked)}/><span>{pc.pc_name}</span></label>):<small>등록된 PC가 없습니다.</small>}</div></td><td><button onClick={()=>save(row)}>저장</button></td></tr>)}</tbody></table></div></div></section>
}
