...
function safeText(value: unknown, fallback: LegacyValue = '-') {
...
function RedisLogicalModel({ database }: { database: DbErdDatabase }) {
  ...
  return <div className="db-erd-logical-grid redis">
    {keys.length ? keys.map((item: LegacyValue, index: LegacyValue) => <div className="db-erd-logical-card" key={`${safeText(item.key, 'key')}:${index}`}>
...
function DocumentLogicalModel({ database }: { database: DbErdDatabase }) {
  ...
  return <div className="db-erd-logical-grid document">
    {collections.length ? collections.map((item: LegacyValue, index: LegacyValue) => <div className="db-erd-logical-card" key={`${safeText(item.name, 'collection')}:${index}`}>
...
export function DatabaseErdPanel({ report, loading, error, onRefresh }: DatabaseErdPanelProps) {
  ...
  const active = useMemo(() => {
    ...
    return databases.find((item: LegacyValue) => String(item.id || item.engine || '') === selectedId) || databases[0]
  ...
  return <div className="db-erd-panel-shell">
    ...
    {databases.length > 0 && <>
      <div className="db-erd-database-tabs">
        {databases.map((item: LegacyValue) => {
    ...
    {(report?.notes || []).length > 0 && <div className="db-erd-notes">{(report?.notes || []).map((note: LegacyValue, index: LegacyValue) => <span key={index}>{note}</span>)}</div>}
  </div>
}
