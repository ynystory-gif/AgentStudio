...
function ensureStyle() {
  ...
  style.textContent = `
    ...
    .ui-layout-dynamic-source-v2 .head b{font-size:12px}.ui-layout-dynamic-source-v2 .head span{font-size:10px;color:#7e9ab2}
...
async function imageReference(file, role) {
  ...
  const primary = palette.find((x) => x !== background && x !== surface) || '#2563eb'
  return {
    file_name: file.name,
...
function renderProgress(host, current, startedAt, { done = false, failed = false } = {}) {
  ...
  const bar = box.querySelector('[data-v2-bar]')
...
function install(panel) {
  ...
  const state = { urls: [''], files: [null], roles: ['default'], busy: false, jobId: '', cancelRequested: false, tickTimer: 0 }
...
>>>>>>> d0e40bd86a999808d857b8acca8a9a6f14259c81
  ...
  const renderRows = () => {
    ...
    if (urlRows) urlRows.innerHTML = state.urls.map((value, index) => `<div class="row"><input type="text" data-url-index="${index}" value="${esc(value)}" placeholder="https://example.com"/><button type="button" data-remove-url="${index}" ${state.urls.length === 1 ? 'disabled' : ''}>×</button></div>`).join('')
    if (imageRows) imageRows.innerHTML = state.files.map((file, index) => `<div class="row image"><input type="file" accept="image/*" data-image-index="${index}"/><select data-role-index="${index}"><option value="default">기본 화면</option><option value="menu_hover">메뉴 Hover</option><option value="user_menu_open">사용자 메뉴 Open</option><option value="active">Active 상태</option></select><button type="button" data-remove-image="${index}" ${state.files.length === 1 ? 'disabled' : ''}>×</button>${file ? `<small>선택: ${esc(file.name)}</small>` : ''}</div>`).join('')
  }
  renderRows()
  ...
  host.addEventListener('click', async (event) => {
    ...
    const ru = target.closest('[data-remove-url]'); if (ru && state.urls.length > 1) { state.urls.splice(Number(ru.dataset.removeUrl), 1); renderRows(); return }
    const ri = target.closest('[data-remove-image]'); if (ri && state.files.length > 1) { const i = Number(ri.dataset.removeImage); state.files.splice(i, 1); state.roles.splice(i, 1); renderRows() }
  })
  ...
  action?.addEventListener('click', async (event) => {
    ...
    const urls = state.urls.map((x) => x.trim()).filter(Boolean)
    const selected = state.files.map((file, index) => ({ file, index })).filter((x) => x.file)
...
export function LayoutThemeDynamicSourceV2() {
  useEffect(() => {
    ensureStyle()
    const scan = () => document.querySelectorAll('.ui-layout-theme-import-panel.unified-source').forEach((panel) => install(panel))
    scan()
