...
function markMobilePreviews(){
  ...
  if(!modal)return
  const active=[...modal.querySelectorAll('.ui-layout-gallery-toolbar button')].find((button: LegacyValue)=>button.classList.contains('active'))
  const mobileCategory=String(active?.textContent||'').trim()==='모바일'
  modal.querySelectorAll('.ui-layout-wireframe').forEach((wireframe: LegacyValue)=>{
...
async function imageReference(file: LegacyValue,role: LegacyValue){
  ...
  const background=palette[0]||'#ffffff',surface=palette[1]||background,primary=palette.find((x: LegacyValue)=>x!==background&&x!==surface)||'#2563eb'
  const rgb=(hex: LegacyValue)=>[parseInt(hex.slice(1,3),16),parseInt(hex.slice(3,5),16),parseInt(hex.slice(5,7),16)]
  const [rr=0,gg=0,bb=0]=rgb(background);const lum=(.2126*rr+.7152*gg+.0722*bb)/255
...
const sleep=(ms: LegacyValue)=>new Promise((resolve: LegacyValue)=>setTimeout(resolve,ms))
const elapsedText=(start: LegacyValue)=>{
...
function setProgress(host: LegacyValue,pct: LegacyValue,message: LegacyValue,stage: LegacyValue,start: LegacyValue,{done=false,error=false,warning=false,stalledSeconds=0,urlTimeout=300,jobTimeout=300,canCancel=true}:LegacyRecord={}){
  const box=host.querySelector('[data-analysis-progress]') as HTMLElement | null
...
async function waitForJob(host: LegacyValue,job: LegacyValue,start: LegacyValue,state: LegacyValue){
  let current=job
...
function render(host: LegacyValue,state: LegacyValue,status: LegacyValue){
...
function install(panel: LegacyValue){
  ...
  const state:{urls:string[];files:(File|null)[];roles:string[];busy:boolean;jobId:string;cancelRequested:boolean}={urls:[''],files:[null],roles:['default'],busy:false,jobId:'',cancelRequested:false}
  ...
  host.addEventListener('click',async (event: LegacyValue)=>{
    ...
    if(target.closest('[data-analysis-cancel]')){
      ...
      const cancelButton=host.querySelector<HTMLButtonElement>('[data-analysis-cancel]')
      ...
      try{await api(`/ui-themes/import-dynamic/jobs/${encodeURIComponent(state.jobId)}/cancel`,{method:'POST'})}catch(error){state.cancelRequested=false;status.textContent=String(asLegacyError(error).message||error);status.className='ui-layout-dynamic-status error'}
      return
    }
  ...
  host.addEventListener('input',(event: LegacyValue)=>{const t=event.target;if(!(t instanceof HTMLInputElement)||state.busy)return;const idx=t.dataset.urlIndex;if(idx!==undefined)state.urls[Number(idx)]=t.value})
  host.addEventListener('change',(event: LegacyValue)=>{if(state.busy)return;const t=event.target;if(t instanceof HTMLInputElement&&t.dataset.imageIndex!==undefined){state.files[Number(t.dataset.imageIndex)]=t.files?.[0]||null;render(host,state,status)}else if(t instanceof HTMLSelectElement&&t.dataset.roleIndex!==undefined){state.roles[Number(t.dataset.roleIndex)]=t.value}})
  ...
  action?.addEventListener('click',async (event: LegacyValue)=>{
    ...
    const name=String(nameInput?.value||'').trim();const urls=state.urls.map((x: LegacyValue)=>x.trim()).filter(Boolean);const selected=state.files.map((file: LegacyValue,index: LegacyValue)=>({file,index})).filter((x: LegacyValue):x is {file:File;index:number}=>x.file instanceof File)
    ...
    Array.from(panel.querySelectorAll('[data-add-url],[data-add-image],[data-remove-url],[data-remove-image],input,select') as NodeListOf<HTMLInputElement|HTMLButtonElement|HTMLSelectElement>).forEach((el: LegacyValue)=>{if(el!==nameInput)el.disabled=true})
    try{
      ...
      status.textContent=String(asLegacyError(error).message||error);status.className='ui-layout-dynamic-status error'
      const box=host.querySelector('[data-analysis-progress]') as HTMLElement | null;if(box)box.style.borderColor='#6f3030'
    }finally{
      state.busy=false;state.jobId='';state.cancelRequested=false
      if(action){action.disabled=false;action.textContent='분석 후 Theme 저장'}
      Array.from(panel.querySelectorAll('[data-add-url],[data-add-image],[data-remove-url],[data-remove-image],input,select') as NodeListOf<HTMLInputElement|HTMLButtonElement|HTMLSelectElement>).forEach((el: LegacyValue)=>{el.disabled=false})
      render(host,state,status)
    }
  },true)
}
...
export function LayoutThemeDynamicSourceEnhancer(){
  useEffect(()=>{
    ensureStyle()
    const scan=()=>{
      document.querySelectorAll('.ui-layout-theme-import-panel.unified-source').forEach((panel: LegacyValue)=>install(panel))
      markMobilePreviews()
    }
    scan()
