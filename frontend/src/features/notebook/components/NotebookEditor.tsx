...
import { notebookKernelLanguage, parseNotebookDocument, textToNotebookSource } from '../notebookUtils'
import type {
  NotebookCell,
  NotebookDocument,
...
function normalizeNotebookLineBreakpoints(value: unknown): NotebookLineBreakpoint[] {
  ...
  value.forEach((item: LegacyValue) => {
  ...
  return Array.from(unique.values()).sort((a: LegacyValue, b: LegacyValue) => a.cellIndex - b.cellIndex || a.lineNumber - b.lineNumber)
}
...
// NotebookEditor is conditionally mounted by App.tsx. Switching from an .ipynb
...
export interface NotebookEditorProps {
  ...
  onNavigateProjectDefinition?: (definition: LegacyValue, sourceLocation?: { line: number; column: number; cellIndex?: number }) => void | Promise<void>
  onExternalDefinitionPreview?: (definition: LegacyValue) => void
}
...
function findNotebookDefinitionCells(notebook: NotebookDocument, symbol: string): number[] {
  ...
  notebook.cells.forEach((cell: LegacyValue, index: LegacyValue) => {
    if (cell?.cell_type !== 'code') return
    const source = notebookSourceToText(cell.source)
    if (patterns.some((pattern: LegacyValue) => pattern.test(source))) matches.push(index)
  })
  return matches
}
...
export function NotebookEditor({
  ...
  const sourceCommitIdleRef = useRef<number | null>(null)
  const sourceCommitPendingRef = useRef(false)
  ...
  useEffect(() => {
    return () => {
      if (sourceCommitPendingRef.current) {
        try { flushPendingSourceChanges() } catch {}
      } else {
        clearSourceCommitTimer()
      }
    }
  }, [filePath])
  ...
  useEffect(() => {
    if (!parsed.ok) return
    const maxCount = (parsed.notebook.cells || []).reduce((max: LegacyValue, cell: LegacyValue) => {
  ...
  useEffect(() => {
    ...
    Object.values(executionProgressDelayRef.current).forEach((timer: LegacyValue) => window.clearTimeout(timer))
  ...
  useEffect(() => {
    if (!parsed.ok) return
    parsed.notebook.cells.forEach((cell: LegacyValue, index: LegacyValue) => {
      const source = notebookSourceToText(cell?.source)
  ...
  const updateBookmarkState = (next: NotebookLineBookmark[]): NotebookLineBookmark[] => {
    ...
    setBookmarkRevision((value: LegacyValue) => value + 1)
    return stored
  }
  ...
  const toggleLineBookmark = (cellIndex: number, lineNumber: number): void => {
    ...
    const exists = current.some((item: LegacyValue) => item.cellIndex === safeCellIndex && item.lineNumber === safeLineNumber)
    updateBookmarkState(exists
      ? current.filter((item: LegacyValue) => !(item.cellIndex === safeCellIndex && item.lineNumber === safeLineNumber))
  ...
  const applyBookmarkDecorations = (cellIndex: number, editor?: NotebookMonacoEditorLike): void => {
    ...
    const nextDecorations: NotebookEditorDecorationLike[] = getNotebookLineBookmarks(scrollKeyRef.current)
      .filter((item: LegacyValue) => item.cellIndex === cellIndex && item.lineNumber <= maxLineNumber)
  ...
  useEffect(() => {
    Object.entries(cellEditorsRef.current).forEach(([indexText, editor]: LegacyValue) => {
      if (!editor) return
  ...
  const updateBreakpointState = (next: NotebookLineBreakpoint[]): NotebookLineBreakpoint[] => {
    ...
    setBreakpointRevision((value: LegacyValue) => value + 1)
    return stored
  }
  ...
  const moveToBookmark = (direction: 1 | -1): void => {
    ...
    if (direction > 0) {
      target = current.find((item: LegacyValue) => item.cellIndex > activeCellIndex || (item.cellIndex === activeCellIndex && item.lineNumber > currentLine))
      target ||= current[0]
    } else {
      target = [...current].reverse().find((item: LegacyValue) => item.cellIndex < activeCellIndex || (item.cellIndex === activeCellIndex && item.lineNumber < currentLine))
  ...
  const shiftBookmarksForInsertedCell = (insertAt: number): void => {
    const current = getNotebookLineBookmarks(scrollKeyRef.current)
    if (!current.length) return
    updateBookmarkState(current.map((item: LegacyValue) => item.cellIndex >= insertAt
  ...
  const shiftBreakpointsForInsertedCell = (insertAt: number): void => {
    ...
    if (!current.length) return
    updateBreakpointState(current.map((item: LegacyValue) => item.cellIndex >= insertAt
      ? { ...item, cellIndex: item.cellIndex + 1 }
      : item))
  }
  ...
  const shiftBookmarksForDeletedCell = (deletedIndex: number): void => {
    ...
    updateBookmarkState(current
      .filter((item: LegacyValue) => item.cellIndex !== deletedIndex)
      .map((item: LegacyValue) => item.cellIndex > deletedIndex
        ? { ...item, cellIndex: item.cellIndex - 1 }
        : item))
  }
  ...
  const buildLiveNotebookContent = (): string => {
    ...
    Object.entries(latestCellSourcesRef.current).forEach(([indexText, source]: LegacyValue) => {
      const index = Number(indexText)
      if (!Number.isInteger(index)) return
      const cell = notebook.cells[index]
      if (!cell || !['code','markdown'].includes(String(cell.cell_type || ''))) return
      notebook.cells[index] = { ...cell, source: textToNotebookSource(String(source ?? '')) }
    })
    return JSON.stringify(notebook, null, 1) + '\n'
  }

  const clearSourceCommitTimer = () => {
    if (sourceCommitTimerRef.current != null) {
      window.clearTimeout(sourceCommitTimerRef.current)
      sourceCommitTimerRef.current = null
    }
    if (sourceCommitIdleRef.current != null && 'cancelIdleCallback' in window) {
      ;(window as LegacyValue).cancelIdleCallback(sourceCommitIdleRef.current)
      sourceCommitIdleRef.current = null
    }
  }

  const flushPendingSourceChanges = (): string => {
    clearSourceCommitTimer()
    const serialized = buildLiveNotebookContent()
    if (sourceCommitPendingRef.current) {
      sourceCommitPendingRef.current = false
      onChange?.(serialized)
    }
    return serialized
  }

  const scheduleSourceCommit = () => {
    sourceCommitPendingRef.current = true
    clearSourceCommitTimer()
    // v5.564: keep the hot keystroke path limited to Monaco + the local ref.
    // Full .ipynb serialization is deferred until a longer pause and, when
    // supported, a browser idle slot. Ctrl+S/blur still flush immediately.
    sourceCommitTimerRef.current = window.setTimeout(() => {
      sourceCommitTimerRef.current = null
      const commit = () => {
        sourceCommitIdleRef.current = null
        if (!sourceCommitPendingRef.current) return
        sourceCommitPendingRef.current = false
        onChange?.(buildLiveNotebookContent())
      }
      if ('requestIdleCallback' in window) {
        sourceCommitIdleRef.current = (window as LegacyValue).requestIdleCallback(commit,{timeout:600})
      } else {
        commit()
      }
    }, 900)
  ...
  const patchCell = (index: number, patch: Partial<NotebookCell>) => {
    ...
    const notebook = structuredClone(parsed.notebook)
    Object.entries(latestCellSourcesRef.current).forEach(([indexText, source]: LegacyValue) => {
      const liveIndex = Number(indexText)
      const cell = notebook.cells[liveIndex]
      if (!Number.isInteger(liveIndex) || !cell) return
      if (!['code','markdown'].includes(String(cell.cell_type || ''))) return
      notebook.cells[liveIndex] = { ...cell, source: textToNotebookSource(String(source ?? '')) }
    })
  ...
  const updateCellSource = (index: number, text: string) => {
    // The active Monaco model is the immediate source of truth. Do not clone and
    // stringify the complete notebook for every character.
    latestCellSourcesRef.current[index] = text
    scheduleSourceCommit()
  }

  const revealCodeDefinition = (cellIndex: number, line: number, column: LegacyValue = 1): void => {
    const maxIndex = parsed.ok ? Math.max(0, parsed.notebook.cells.length - 1) : 0
  ...
  const notebookCurrentCodeLocation = (fallbackCellIndex: LegacyValue = activeCellIndex): { cellIndex: number; line: number; column: number } => {
  ...
  const openNotebookDefinition = async (definition: LegacyValue, sourceLocation?: { line: number; column: number }, sourceCellIndex: LegacyValue = activeCellIndex): Promise<void> => {
  ...
  const applyExecutionResult = (
    ...
    if (Array.isArray(result?.rich_outputs)) {
      result.rich_outputs.forEach((output: LegacyValue) => {
    ...
    if (result?.cancelled) {
      ...
      if (missingSymbol) {
        ...
        const candidateText = definitionCells.length
          ? `정의 후보 셀: ${definitionCells.map((cellIndex: LegacyValue) => `Cell ${cellIndex + 1}`).join(', ')}`
  ...
  const handleLiveOutputEvent = (index: number, event: NotebookLiveOutputEvent): void => {
    ...
    if (eventName === 'clear_output') {
      if (Boolean(event?.wait)) {
        ...
        setLiveOutputsByCell((prev: LegacyValue) => ({ ...prev, [index]: [] }))
      }
      return
    }
    ...
    setLiveOutputsByCell((prev: LegacyValue) => {
      ...
      if (eventName === 'update_display_data' && event.display_id) {
        ...
        const found = current.findIndex((item: LegacyValue) => String((item as LegacyValue)?.transient?.display_id || '') === displayId)
        const nextOutput = { ...output, transient: { ...((output as LegacyValue).transient || {}), display_id: displayId } }
  ...
  const executeCellFromNotebook = async (
    ...
    setRunningCells((prev: LegacyValue) => ({ ...prev, [index]: true }))
    executionStartedAtRef.current[index] = Date.now()
    setExecutionHeartbeatAt(Date.now())
    setExecutionProgressVisible((prev: LegacyValue) => ({ ...prev, [index]: false }))
    ...
    executionProgressDelayRef.current[index] = window.setTimeout(() => {
      setExecutionProgressVisible((prev: LegacyValue) => ({ ...prev, [index]: true }))
    }, 650)
    pendingLiveClearWaitRef.current[index] = false
    ...
    setLiveOutputsByCell((prev: LegacyValue) => ({ ...prev, [index]: previousOutputs }))
    try {
      const result = await onExecutePython?.({
        ...
        selectionOnly,
        onOutputEvent: (event: LegacyValue) => handleLiveOutputEvent(index, event),
      })
      ...
      setExecutionProgressVisible((prev: LegacyValue) => {
        if (!Object.prototype.hasOwnProperty.call(prev, index)) return prev
        const next = { ...prev }
        delete next[index]
        return next
      })
      setRunningCells((prev: LegacyValue) => ({ ...prev, [index]: false }))
      pendingLiveClearWaitRef.current[index] = false
      replaceLiveOutputOnNextEventRef.current[index] = false
  ...
  const runAll = async (): Promise<void> => {
    ...
    try {
      ...
      for (let index = 0; index < working.cells.length; index += 1) {
        ...
        if (working.cells[index]?.outputs?.some((output: LegacyValue) => output.output_type === 'error')) break
      }
    } finally {
      setRunAllBusy(false)
    }
  ...
  const handleDebugResponse = (state: NotebookDebugResult | null | undefined): void => {
    ...
    if (event === 'evaluate') {
      ...
      if (expression) {
        setDebugConsole((prev: LegacyValue) => [...prev, { expression, result, error: !!error }].slice(-50))
      }
      setDebugState((prev: LegacyValue) => ({ ...(prev || {}), ...state, debug_active: true }))
      return
    }
    setDebugState(state)
  ...
  const startDebugCell = async (index: number): Promise<void> => {
    ...
    const lines = getNotebookLineBreakpoints(scrollKeyRef.current)
      .filter((item: LegacyValue) => item.cellIndex === index)
      .map((item: LegacyValue) => item.lineNumber)
    setDebugBusy(true)
    setDebugConsole([])
    setActiveCellIndex(index)
    try {
  ...
  const sendDebugCommand = async (command: NotebookDebugCommandRequest['command'], expression: LegacyValue = ''): Promise<void> => {
  ...
  const revealSearchMatch = (cellIndex: number, lineNumber: LegacyValue = 1, column: LegacyValue = 1, length: LegacyValue = 1): void => {
    const maxIndex = parsed.ok ? Math.max(0, parsed.notebook.cells.length - 1) : 0
  ...
  const addCell = (cellType: 'code' | 'markdown') => {
    ...
    if (cellType === 'markdown') setEditingMarkdown((prev: LegacyValue) => ({ ...prev, [insertAt]: true }))
  }
  ...
  const clearAllOutputs = () => {
    if (!parsed.ok) return
    const notebook = structuredClone(parsed.notebook)
    notebook.cells = notebook.cells.map((cell: LegacyValue) => cell?.cell_type === 'code'
  ...
  if (!parsed.ok) {
    ...
    return <div
      ...
      onScroll={(event: LegacyValue) => rememberOuterScroll(event.currentTarget.scrollTop)}
    >
      ...
      <Editor
        ...
        language="json"
        value={value}
        onChange={(nextValue: LegacyValue) => onChange?.(nextValue ?? '')}
        theme="vs-dark"
  ...
  >
    ...
    {(debugState?.debug_active || breakpoints.length > 0) && <div className="notebook-debug-toolbar" role="region" aria-label="Notebook 디버그 도구">
      ...
      <div className="notebook-debug-toolbar-actions">
        {!debugState?.debug_active && <button type="button" className={debugBusy ? 'busy' : ''} disabled={debugBusy || !isPythonNotebookKernel(kernel)} title="중단점이 있는 셀 또는 현재 셀에서 디버그 시작" onClick={() => {
          const activeHasBreakpoint = breakpoints.some((item: LegacyValue) => item.cellIndex === activeCellIndex)
    ...
    <div className="notebook-cells">
      {notebook.cells.map((cell: LegacyValue, index: LegacyValue) => {
          ...
          <div className="notebook-cell-main">
            <div className="notebook-cell-toolbar">
              ...
              <div>
                ...
                {cellType === 'markdown' && <button type="button" onClick={(event: React.MouseEvent<HTMLButtonElement>) => { event.stopPropagation(); setEditingMarkdown((prev: LegacyValue) => ({ ...prev, [index]: !prev[index] })) }}>{editingMarkdown[index] ? '미리보기' : '편집'}</button>}
            ...
            {cellType === 'code'
              ? <div
                ...
                >
                  ...
                  language="python"
                  defaultValue={source}
                  onChange={(nextValue: LegacyValue) => updateCellSource(index, nextValue ?? '')}
                  onMount={(editor: LegacyValue, monaco: LegacyValue) => {
                    ...
                    registerEscapedDoubleQuotePairGuard(editor as unknown as LegacyValue)
                    registerCodeIntelligence(monaco, editor, {
                      ...
                      onOpenDefinition: (definition: LegacyValue, sourceLocation: LegacyValue) => openNotebookDefinition(definition, sourceLocation, index),
                    })
              ...
              : cellType === 'markdown'
                ? (editingMarkdown[index]
                    ? <Editor
                      ...
                      />
                    : <div className="notebook-markdown-cell" onDoubleClick={() => setEditingMarkdown((prev: LegacyValue) => ({ ...prev, [index]: true }))}><NotebookMarkdown text={source} attachments={cell.attachments || {}} /></div>)
            ...
            {cellType === 'code' && debugState?.debug_active && Number(debugState.cell_index) === index && <div className="notebook-debug-panel">
              ...
              <div className="notebook-debug-grid">
                <section>
                  ...
                  <div className="notebook-debug-variable-list">
                    ...
                    {(debugState.variables || []).map((item: LegacyValue, itemIndex: LegacyValue) => <div className="notebook-debug-variable" key={`${item.name}-${itemIndex}`}>
                ...
                <section>
                  ...
                  <div className="notebook-debug-stack-list">
                    {(debugState.stack || []).map((frame: LegacyValue, frameIndex: LegacyValue) => <button type="button" key={`${frame.function}-${frame.line}-${frameIndex}`} onClick={() => revealDebugLine({ ...debugState, line: frame.line })}>
              ...
              <div className="notebook-debug-console">
                ...
                {debugConsole.length > 0 && <div className="notebook-debug-console-history">
                  {debugConsole.map((item: LegacyValue, itemIndex: LegacyValue) => <div key={`${item.expression}-${itemIndex}`} className={item.error ? 'error' : ''}><span>› {item.expression}</span><code>{item.result}</code></div>)}
                </div>}
                <div className="notebook-debug-console-input">
                  <input value={debugExpression} onChange={(event: LegacyValue) => setDebugExpression(event.target.value)} placeholder="예: all_tokens, len(all_tokens), word_counts" onKeyDown={(event: LegacyValue) => { if (event.key === 'Enter' && debugExpression.trim()) { event.preventDefault(); void sendDebugCommand('evaluate', debugExpression.trim()) } }} />
            ...
            {cellType === 'code' && (() => {
              ...
              return (
                <div className={`notebook-cell-outputs${hasLiveOutputs ? ' live' : ''}`}>
                  {visibleOutputs.map((output: LegacyValue, outputIndex: LegacyValue) => <NotebookOutput key={outputIndex} output={output} />)}
                </div>
              )
            })()}
          </div>
