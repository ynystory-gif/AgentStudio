...
export function SqlResultsPane({
  ...
  const beginColumnResize = (event: ReactPointerEvent<HTMLSpanElement>, columnIndex: number) => {
    ...
    const onMove = (moveEvent: PointerEvent) => {
      ...
      setColumnWidths((previous: LegacyValue) => ({ ...previous, [key]: next }))
    }
    ...
    <div className="sql-results-body">
      {resultTab === 'DATA'
        ? (activeResult?.columns?.length
            ? <div className={`sql-data-output ${resultSets.length > 1 ? 'multi' : 'single'}`}>
                {resultSets.length > 1 && <div className="sql-result-set-tabs" role="tablist" aria-label="SELECT 실행 결과 목록">
                  {resultSets.map((item: LegacyValue, index: LegacyValue) => <button
                    type="button"
                    role="tab"
                ...
                <div className="sql-data-table-wrap">
                  <table className="sql-data-table">
                    <thead><tr>
                      ...
                      {activeResult.columns.map((column: LegacyValue, index: LegacyValue) => <th key={`${column}-${index}`} style={columnStyle(index)}>
                    ...
                    <tbody>{activeResult.rows.map((row: LegacyValue, rowIndex: LegacyValue) => <tr key={rowIndex}>
                      <td className="row-index">{rowIndex + 1}</td>
                      {activeResult.columns.map((_: LegacyValue, cellIndex: LegacyValue) => {
        ...
        : <div className="sql-message-list">
            {messages.length
              ? messages.map((item: LegacyValue, index: LegacyValue) => <div className={`sql-message ${item.type || 'info'}`} key={`${item.time}-${index}`}><span>{item.time}</span><p>{item.text}</p></div>)
