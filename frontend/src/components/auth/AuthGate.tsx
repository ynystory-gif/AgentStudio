...
export function AuthGate({children}:{children:ReactNode}){
  ...
  const initials=useMemo(()=>{
    const name=String(member?.name||member?.login_id||'?').trim()
  ...
  return <div className="auth-root">
    <div className="auth-user-menu" ref={menuRef}>
      <button className={`auth-avatar-button ${menuOpen?'active':''}`} type="button" onClick={async()=>{const next=!menuOpen;setMenuOpen(next);if(next){try{await loadPcStatus()}catch{}}}} title={`${member.name} 사용자 메뉴`} aria-haspopup="menu" aria-expanded={menuOpen}>{initials}</button>
      {menuOpen&&<div className="auth-user-dropdown" role="menu">
        <div className="auth-user-summary"><span className="auth-avatar-large">{initials}</span><div><strong>{member.name}</strong><small>{member.login_id} · {member.role}</small></div></div>
