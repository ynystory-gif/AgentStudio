...
export function normalizeNotebookSqlCode(source = ''): string {
...
export function looksLikeNotebookSqlCode(source = ''): boolean {
...
export function textToNotebookSource(text = ''): string[] {
  const normalized = String(text ?? '').replace(/\r\n|\r/g, '\n')
  if (!normalized) return []
  const parts = normalized.split('\n')
  return parts
    .map((line, index) => index < parts.length - 1 ? `${line}\n` : line)
    .filter((line, index) => line !== '' || index < parts.length - 1)
}

export function parseNotebookDocument(value = ''): NotebookParseResult {
  try {
