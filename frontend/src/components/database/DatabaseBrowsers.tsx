...
function subscribeRedisTtlClock(listener: () => void): () => void {
  ...
  if (redisTtlClockTimer === null && typeof window !== 'undefined') {
    ...
    redisTtlClockTimer = window.setInterval(() => {
      redisTtlClockNow = Date.now()
      redisTtlClockListeners.forEach(current => current())
    }, 1000)
  }
  return () => {
...
export function FirestoreBrowserPanel({
  ...
  const visibleCollections = collectionNeedle
    ? collections.filter(item => String(item?.id || '').toLowerCase().includes(collectionNeedle))
    : collections
  ...
  const visibleDocuments = documentNeedle
    ? documentRows.filter(item => String(item?.id || '').toLowerCase().includes(documentNeedle))
    : documentRows
  ...
  return <div className="firestore-browser-shell">
    <div className="firestore-browser-toolbar">
      <div className="firestore-filter-box">
        ...
        <input value={collectionFilter} onChange={e => setCollectionFilter(e.target.value)} placeholder="컬렉션 검색" />
      </div>
      <div className="firestore-filter-box">
        <span>Document</span>
        <input value={documentFilter} onChange={e => setDocumentFilter(e.target.value)} placeholder="문서 ID 검색" />
      </div>
    </div>
              ...
              : visibleCollections.length
                ? visibleCollections.map(item => {
                    ...
                    return <button
                      ...
                      onContextMenu={event => openContextMenu(event, { kind: 'collection', path, label: item.id })}
                ...
                : visibleDocuments.length
                  ? visibleDocuments.map(item => {
                      ...
                      return <button
                        ...
                        onContextMenu={event => openContextMenu(event, { kind: 'document', path, label: item.id })}
                ...
                <div className="firestore-detail-content">
                  {(detail.fields || []).length
                    ? <div className="firestore-field-grid">
                        ...
                        {(detail.fields || []).map((field, index) => <div className="firestore-field-row" key={`${field.name}-${index}`}>
                  ...
                  {(detail.subcollections || []).length > 0 && <div className="firestore-subcollections"><strong>Subcollections</strong>{(detail.subcollections || []).map(item => <span key={item.path}>{item.id}</span>)}</div>}
                </div>
...
export function RedisBrowserPanel({
  ...
  const visibleKeys = typeFilter === 'all' ? allKeys : allKeys.filter(item => String(item?.type || '').toLowerCase() === typeFilter)
  const tree = buildRedisKeyTree(visibleKeys)
  const detail = keyDetail
  const detailType = String(detail?.type || '').toLowerCase()

  const renderTreeNodes = (node: RedisTreeNode | null | undefined, level = 0): React.ReactNode => {
    if (!node) return null
    return <>
      {(node.children || []).map(child => {
        ...
        return <div className="redis-tree-group" key={`redis-group:${child.path}`}>
          <button
            ...
            onContextMenu={event => openContextMenu(event, { kind: 'group', prefix: child.path, label: child.path })}
      ...
      {(node.items || []).map(item => {
        ...
        return <button
          ...
          onContextMenu={event => openContextMenu(event, { kind: 'key', key: fullKey, keyType: type, prefix: parentPrefix, label: fullKey })}
  ...
  return <div className="redis-browser-shell">
    <div className="redis-browser-toolbar">
      <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} title="Redis Key 타입 필터">
      ...
      <div className="redis-key-search">
        <input
          ...
          onKeyDown={e => { if (e.key === 'Enter') loadKeys({ preserveSelection: false }) }}
            ...
            : <>
                ...
                <div className="redis-detail-content">
                  {detailType === 'string'
                    ...
                    : detailType === 'hash'
                      ? <table className="redis-value-table"><thead><tr><th>Field</th><th>Value</th></tr></thead><tbody>{(detail.rows || []).map((row, index) => <tr key={`${row.field}-${index}`}><td>{row.field}</td><td>{String(row.value ?? '')}</td></tr>)}</tbody></table>
                      : ['list', 'set'].includes(detailType)
                        ? <table className="redis-value-table"><thead><tr><th>Index</th><th>Element</th></tr></thead><tbody>{(detail.rows || []).map((row, index) => <tr key={`${row.index}-${index}`}><td>{row.index}</td><td>{String(row.value ?? '')}</td></tr>)}</tbody></table>
                        : detailType === 'zset'
...
export function SqlObjectTreePanel({
  ...
  return <div className="sql-object-tree">
    ...
    {dbObjects.schemas.map((schema: SqlObjectSchema, schemaIndex) => {
                ...
                {items.map((item, index) => {
                  ...
                  return <div className="sql-object-item" key={itemKey}>
                    <button
                      ...
                      onContextMenu={event => openObjectContextMenu(event, schemaName, category, item)}
...
export function DatabaseBrowserContextMenus({
  ...
  return <>
    ...
    >
      ...
      ] as const).map(([action, icon, title, description]) => <button
        type="button"
        key={action}
      ...
      <div className={`sql-admin-prompt ${sqlAdminPrompt.danger ? 'danger' : ''}`} onMouseDown={event => event.stopPropagation()}>
        ...
        <label>
          ...
          <input
            ...
            onChange={event => setSqlAdminPrompt(prev => prev ? ({ ...prev, value: event.target.value }) : prev)}
            onKeyDown={event => {
              if (event.key === 'Enter') {
                event.preventDefault()
                submitSqlAdminPrompt()
