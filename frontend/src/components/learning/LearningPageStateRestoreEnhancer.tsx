...
const tabFromButton = (button: LegacyValue) => {
...
const buttonForTab = (tab: LegacyValue) => {
  const buttons = Array.from(document.querySelectorAll('.llm-learning-toolbar > button'))
  return buttons.find((button: LegacyValue) => tabFromButton(button) === tab) || null
}
...
export function LearningPageStateRestoreEnhancer() {
  useEffect(() => {
    ...
    const rememberClick = (event: LegacyValue) => {
    ...
    const restore = () => {
      ...
      const button = buttonForTab(tab) as HTMLButtonElement | null
