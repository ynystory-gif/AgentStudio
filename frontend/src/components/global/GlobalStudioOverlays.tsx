...
const statusLabel=(value: LegacyValue='')=>{
...
export function GlobalCommandPalette({open,onClose,commands=[]}:LegacyRecord){
  const [query,setQuery]=useState('')
  const [selectedIndex,setSelectedIndex]=useState(0)
  const inputRef=useRef<LegacyValue|null>(null)
  const filtered=useMemo(()=>{
    const needle=query.trim().toLowerCase()
    const source=Array.isArray(commands)?commands:[]
    if(!needle) return source
    return source.filter((command: LegacyValue)=>[
  ...
  if(!open) return null
  const runCommand=(command: LegacyValue)=>{
    if(!command||command.disabled) return
    onClose?.()
    window.setTimeout(()=>command.run?.(),0)
  }
  return <div className="studio-overlay command-palette-overlay" onMouseDown={(event: LegacyValue)=>{if(event.target===event.currentTarget) onClose?.()}}>
    <div className="command-palette" role="dialog" aria-modal="true" aria-label="전역 명령 팔레트">
      <div className="command-palette-search">
        ...
        <input
          ref={inputRef}
          value={query}
          onChange={(event: LegacyValue)=>{setQuery(event.target.value);setSelectedIndex(0)}}
          onKeyDown={(event: LegacyValue)=>{
            if(event.key==='ArrowDown'){event.preventDefault();setSelectedIndex((index: LegacyValue)=>Math.min(filtered.length-1,index+1))}
            if(event.key==='ArrowUp'){event.preventDefault();setSelectedIndex((index: LegacyValue)=>Math.max(0,index-1))}
      ...
      <div className="command-palette-list">
        {filtered.map((command: LegacyValue,index: LegacyValue)=><button
          type="button"
          key={command.id}
...
export function AgentWorkCenterPanel({open,onClose,jobs={},developmentProgress,workflowProgress,redevelopmentInfo,onOpenRun,onRedevelop,onCancelJob}:LegacyRecord){
  const [tab,setTab]=useState('CURRENT')
  if(!open) return null
  const all=(Object.values(jobs||{}) as LegacyRecord[]).filter(Boolean).sort((a: LegacyValue,b: LegacyValue)=>String(b.updated_at||b.created_at||b.id||'').localeCompare(String(a.updated_at||a.created_at||a.id||''))).slice(0,20)
  const active=all.filter((job: LegacyValue)=>['QUEUED','PENDING','RUNNING','WAITING_USER'].includes(String(job.status||'').toUpperCase()))
  const failed=all.filter((job: LegacyValue)=>['FAILED','ERROR','REPAIR_PLAN_INCOMPLETE','TEST_FAILED','TEST_REPAIR_PLAN_INCOMPLETE'].includes(String(job.status||'').toUpperCase()))
  const visible=tab==='CURRENT'?active:tab==='FAILED'?failed:all
  return <div className="studio-side-overlay" onMouseDown={(event: LegacyValue)=>{if(event.target===event.currentTarget) onClose?.()}}>
    <aside className="studio-side-panel agent-work-center" role="dialog" aria-modal="true" aria-label="Agent 작업 센터">
      ...
      <div className="work-center-list">
        {visible.map((job: LegacyValue)=><div className="work-center-job" key={job.id}>
...
const HELP_ARTICLES=[
  ...
  {id:'tabs',category:'탭별 사용법',title:'워크스페이스 탭',keywords:'워크플로우 코드 실행 분석 아키텍처 erd 스케줄러 scheduler llm 브라우저',body:'에이전트 설계, 워크플로우, 실행 결과, 코드 편집, 분석 리포트, 아키텍처, DB ERD, 스케줄러, LLM 리스트, 웹브라우저 탭을 프로젝트 상태에 맞게 사용할 수 있습니다.'},
...
export function HelpCenterPanel({open,onClose,onStartNewAgent,onOpenCommandPalette}:LegacyRecord){
  const [query,setQuery]=useState('')
  if(!open) return null
  const needle=query.trim().toLowerCase()
  const articles=HELP_ARTICLES.filter((article: LegacyValue)=>!needle||`${article.category} ${article.title} ${article.keywords} ${article.body}`.toLowerCase().includes(needle))
  return <div className="studio-side-overlay" onMouseDown={(event: LegacyValue)=>{if(event.target===event.currentTarget) onClose?.()}}>
    <aside className="studio-side-panel help-center-panel" role="dialog" aria-modal="true" aria-label="AgentStudio 사용 방법">
      ...
      <div className="help-search"><span>⌕</span><input value={query} onChange={(event: LegacyValue)=>setQuery(event.target.value)} placeholder="예: 재개발, PPT, ERD, 첨부파일" autoFocus/></div>
      ...
      <div className="help-article-list">
        {articles.map((article: LegacyValue)=><article key={article.id} className="help-article">
