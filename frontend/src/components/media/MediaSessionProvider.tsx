...
function formatTranscript(segments: TranscriptSegment[], interimText: LegacyValue = '', interimSegment: TranscriptSegment | null = null) {
  ...
  const rows = [...segments]
    .sort((a: LegacyValue, b: LegacyValue) => Number(a.offsetMs || 0) - Number(b.offsetMs || 0))
    .map((segment: LegacyValue) => `[${formatOffset(segment.offsetMs)}] ${segment.text}`)
...
function rangesOverlap(a: TranscriptSegment, b: TranscriptSegment, toleranceMs: LegacyValue = 160) {
...
function replaceTranscriptRange(
  ...
  const retained = current.filter((segment: LegacyValue) => {
  ...
  return [...retained, ...replacements]
    .filter((segment: LegacyValue) => String(segment.text || '').trim())
    .sort((a: LegacyValue, b: LegacyValue) => Number(a.offsetMs || 0) - Number(b.offsetMs || 0))
}

function getSpeechRecognitionConstructor() {
  if (typeof window === 'undefined') return null
  return (window as LegacyValue).SpeechRecognition || (window as LegacyValue).webkitSpeechRecognition || null
}
...
function downsampleToPcm16(input: Float32Array, inputSampleRate: number, outputSampleRate: LegacyValue = TARGET_STT_SAMPLE_RATE) {
...
export function MediaSessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const recognitionRef = useRef<LegacyValue>(null)
  ...
  const startSpeechRecognitionFallback = useCallback((language: string, reason: LegacyValue = '') => {
    ...
    const createAndStart = () => {
      ...
      recognition.onresult = (event: LegacyValue) => {
        let interim = ''
        ...
        if (finals.length) {
          ...
          setTranscriptSegments((previous: LegacyValue) => {
      ...
      recognition.onerror = (event: LegacyValue) => {
      ...
      recognition.onend = () => {
        if (!sessionActiveRef.current || !enableSttRef.current || sourceTypeRef.current !== 'MICROPHONE') return
        setSttReconnectCount((previous: LegacyValue) => previous + 1)
  ...
  const startBackendStt = useCallback(async (stream: MediaStream, language: string) => {
    ...
    const opened = await new Promise<boolean>((resolve: LegacyValue) => {
      let settled = false
    ...
    socket.onmessage = (event: LegacyValue) => {
      ...
      if (type === 'final') {
        ...
        setLastRecognizedAt(nowIso())
        setInterimText('')
        setInterimSegment((current: LegacyValue) => current && rangesOverlap(current, segment) ? null : current)
        setTranscriptSegments((previous: LegacyValue) => {
          if (previous.some((existing: LegacyValue) => existing.id === segment.id)) return previous
      ...
      if (type === 'refined') {
        const segments: TranscriptSegment[] = Array.isArray(payload?.segments)
          ? payload.segments.map((item: LegacyValue) => ({
              id: String(item?.id || makeSegmentId()),
        ...
        const replacementEndMs = Number(
          payload?.rangeEndMs
          ?? payload?.durationMs
          ?? segments.reduce((maximum: LegacyValue, item: LegacyValue) => Math.max(maximum, Number(item.endOffsetMs ?? item.offsetMs ?? 0)), 0)
        )
    ...
    const AudioContextCtor = (window.AudioContext || (window as LegacyValue).webkitAudioContext) as typeof AudioContext | undefined
    if (!AudioContextCtor) {
      try { socket.close() } catch {}
    ...
    try {
      ...
      processor.onaudioprocess = (event: LegacyValue) => {
        ...
        if (socket.bufferedAmount > 8 * 1024 * 1024) {
          setSttDroppedChunks((previous: LegacyValue) => previous + 1)
          return
        }
  ...
  const start = useCallback(async (options: StartMediaSessionOptions) => {
    ...
    setStatus('STARTING')
    setError('')
    setRecordingUrl((previous: LegacyValue) => {
    ...
    try {
      const stream = options.sourceType === 'SCREEN'
        ? await (navigator.mediaDevices as LegacyValue).getDisplayMedia({
            video: true,
            audio: true,
      ...
      if (options.sourceType === 'SCREEN' && options.enableStt && stream.getAudioTracks().length === 0) {
        stream.getTracks().forEach((track: LegacyValue) => {
      ...
      const mimeType = preferredTypes.find((type: LegacyValue) => typeof MediaRecorder !== 'undefined' && MediaRecorder.isTypeSupported?.(type)) || ''
      const recorder = mimeType ? new MediaRecorder(stream, { mimeType }) : new MediaRecorder(stream)
      mediaChunksRef.current = []
      recorder.ondataavailable = (event: LegacyValue) => {
        if (event.data && event.data.size > 0) mediaChunksRef.current.push(event.data)
      }
      recorder.onerror = (event: LegacyValue) => setError(`녹음 오류: ${String((event as LegacyValue)?.error?.message || 'MediaRecorder 오류')}`)
      ...
      await stopPcmCapture()
      mediaStreamRef.current?.getTracks().forEach((track: LegacyValue) => track.stop())
  ...
  const stop = useCallback(async () => {
    ...
    mediaStreamRef.current?.getTracks().forEach((track: LegacyValue) => {
      try { track.stop() } catch {}
    })
    mediaStreamRef.current = null
    ...
    if (canRefine && socket) {
      ...
      const waitForRefine = new Promise<void>((resolve: LegacyValue) => {
  ...
  const loadProjectTranscript = useCallback(async (root: string) => {
    ...
    try {
      const result = await api<LegacyValue>(`/project-live-transcript?root=${encodeURIComponent(normalizedRoot)}`)
      ...
      const segments: TranscriptSegment[] = Array.isArray(session.segments)
        ? session.segments.map((item: LegacyValue) => ({
            id: String(item?.id || makeSegmentId()),
            text: String(item?.text || ''),
      ...
      setInterimText('')
      setInterimSegment(null)
      const restoredEngine = String(session.sttEngine || (segments.some((item: LegacyValue) => item.source === 'faster-whisper') ? 'faster-whisper' : '대기'))
      setSttEngine(restoredEngine)
      sttEngineRef.current = restoredEngine
      const restoredRefineStatus = String(session.refineStatus || '').toUpperCase() === 'DONE' || segments.some((item: LegacyValue) => item.refined)
