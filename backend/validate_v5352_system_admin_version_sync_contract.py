...
require("$FallbackAgentStudioVersion = \"5.369\"" in PS1, "launcher fallback version must be 5.369")
...
require('"version": "5.369"' in ROUTES, "health endpoint version must be 5.369")

main_match = re.search(r'FastAPI\s*\([\s\S]*?version\s*=\s*[\"\'](?P<v>\d+\.\d+)[\"\']', MAIN)
health_match = re.search(r'\"version\"\s*:\s*\"(?P<v>\d+\.\d+)\"', ROUTES)
require(main_match is not None and main_match.group("v") == "5.369", "could not resolve local backend version")
require(health_match is not None and health_match.group("v") == main_match.group("v"), "backend health version drift")

print("PASS v5.369 SYSTEM_ADMIN Launcher Version Sync contract")
