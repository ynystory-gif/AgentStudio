...
export function LearningCollectionLimitEnhancer() {
  useEffect(() => {
    ...
    const visibleCaseCount = () => {
      ...
      const actualRows = rows.filter((row: LegacyValue) => row.querySelector('td'))
      return Math.min(10, actualRows.length)
    }

    const refreshLabels = () => {
      const count = visibleCaseCount()
      const buttons = Array.from(document.querySelectorAll<HTMLButtonElement>('.llm-learning-toolbar button'))
      const problemButton = buttons.find((button: LegacyValue) => String(button.textContent || '').includes('문제 수집'))
      if (problemButton) {
     ...
     */
    const onClickCapture = (event: LegacyValue) => {
      ...
      const patchedPrompt = (message: LegacyValue, defaultValue: LegacyValue) => {
