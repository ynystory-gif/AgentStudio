...
assert "AGENTSTUDIO_FRONTEND_VERSION='5.595'" in app
assert 'version="5.595"' in main
assert '"version": "5.595"' in routes
...
# No RAG FK may target a legacy bare .id physical column.
assert not re.search(r'ForeignKey\("rag_[a-z0-9_]+\.id"\)', models), 'legacy RAG FK target .id remains'

# Existing v5.588-v5.594 DBs must migrate in-place without dropping data.
for table, pk in expected.items():
...
# Every built-in module registry table is normalized before generated Agent DDL.
registry_tables = []
...
print('[PASS] v5.595 table-specific Primary Key naming policy contracts')
