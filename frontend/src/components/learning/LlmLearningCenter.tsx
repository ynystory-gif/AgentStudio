...
export function LlmLearningCenter() {
  ...
  const startProblemCollection=async()=>{
    ...
    setMessage('')
    try{
      const job=await api<any>('/learning/problems/collect-job',{method:'POST',body:JSON.stringify({target_per_case:targetPerCase,max_cases:20,provider:'ollama'})})
      setProblemJob(job)
  ...
  const page=open?createPortal(<div className="nav-page-shell llm-learning-page" aria-label="LLM 학습 센터">
    ...
    <div className="llm-problem-help"><b>문제 수집:</b> 오판 주제 하나당 생성할 문제 수만 정하면 됩니다. 아직 Dataset이 없는 확정 오판 주제를 최신순으로 자동 선택하므로 별도의 “오판 범위 수” 입력은 없습니다.</div>
