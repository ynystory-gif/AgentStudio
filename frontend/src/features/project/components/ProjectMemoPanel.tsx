...
type Props = {
  ...
  activeFile?: string
  projectFiles?: string[]
  onAddLlmReference?: (reference: LegacyValue) => boolean | void
...
export function ProjectMemoPanel({ projectRoot, activeFile = '', projectFiles = [], onAddLlmReference, onOpenExternalMedia }: Props) {
  ...
  const [liveSummaryErrorLogPath, setLiveSummaryErrorLogPath] = useState('')
  const [liveSummaryFallback, setLiveSummaryFallback] = useState(false)
  ...
  const [recordingHistoryLoading, setRecordingHistoryLoading] = useState(false)
  const [selectedRecordingId, setSelectedRecordingId] = useState('')
  const [selectedRecordingTranscript, setSelectedRecordingTranscript] = useState('')
  const [selectedRecordingSummary, setSelectedRecordingSummary] = useState('')
  const [recordingHistoryError, setRecordingHistoryError] = useState('')
  ...
  const previousMediaStatusRef = useRef(mediaSession.status)
  ...
  const selectableFiles = useMemo(() => {
    ...
    return Array.from(unique).sort((a: LegacyValue, b: LegacyValue) => a.localeCompare(b, 'ko'))
  }, [normalizedActiveFile, projectFiles])

  const selectedMemo = useMemo(
    () => memos.find((item: LegacyValue) => item.id === selectedId) || null,
    [memos, selectedId]
  )

  const memoForActiveFile = useMemo(
    () => normalizedActiveFile ? memos.find((item: LegacyValue) => memoFileKey(item.filePath) === memoFileKey(normalizedActiveFile)) || null : null,
    [memos, normalizedActiveFile]
  )

  const usedFileKeys = useMemo(
    () => new Set(memos.map((item: LegacyValue) => memoFileKey(item.filePath))),
    [memos]
  )

  const memoTargetFiles = useMemo(() => {
    if (selectedMemo) return [normalizePath(selectedMemo.filePath)]
    return selectableFiles.filter((path: LegacyValue) => !usedFileKeys.has(memoFileKey(path)))
  }, [selectableFiles, selectedMemo, usedFileKeys])

  const currentRecordingId = useMemo(() => {
    const started = String(mediaSession.startedAt || '').trim()
    if (!started) return ''
    return `rec-${started.replace(/[^0-9A-Za-z]+/g, '-').replace(/^-|-$/g, '')}`.slice(0, 150)
  }, [mediaSession.startedAt])

  const loadRecordingHistory = async (preferredId = '') => {
    if (!projectRoot) return
    setRecordingHistoryLoading(true)
    setRecordingHistoryError('')
    try {
      const result = await api<{ recordings?: RecordingHistoryItem[] }>(`/media-stt/history?root=${encodeURIComponent(projectRoot)}`)
      const rows = Array.isArray(result?.recordings) ? result.recordings : []
      setRecordingHistory(rows)
      if (preferredId && rows.some((row: RecordingHistoryItem) => row.recording_id === preferredId)) {
        setSelectedRecordingId(preferredId)
      }
    } catch (historyError) {
      setRecordingHistoryError(`이전 녹음 기록 불러오기 실패: ${String((historyError as Error)?.message || historyError)}`)
    } finally {
      setRecordingHistoryLoading(false)
    }
  }

  const openRecordingHistory = async (recordingId: string) => {
    if (!projectRoot || !recordingId) return
    setSelectedRecordingId(recordingId)
    setSelectedRecordingTranscript('')
    setSelectedRecordingSummary('')
    setRecordingHistoryError('')
    try {
      const result = await api<{ transcript?: string; summary?: string }>(`/media-stt/history/${encodeURIComponent(recordingId)}?root=${encodeURIComponent(projectRoot)}`)
      setSelectedRecordingTranscript(String(result?.transcript || '').trim())
      setSelectedRecordingSummary(String(result?.summary || '').trim())
    } catch (historyError) {
      setRecordingHistoryError(`녹음 기록 열기 실패: ${String((historyError as Error)?.message || historyError)}`)
    }
  }
  ...
  useEffect(() => {
    ...
    void loadMemos(projectRoot)
      .then((next: LegacyValue) => {
        if (cancelled) return
        setMemos(next)
        const current = normalizedActiveFile
          ? next.find((item: LegacyValue) => memoFileKey(item.filePath) === memoFileKey(normalizedActiveFile)) || null
          : null
        if (current) {
  ...
  useEffect(() => {
    ...
    const existing = memos.find((item: LegacyValue) => memoFileKey(item.filePath) === memoFileKey(normalizedActiveFile)) || null
    if (existing) {
  ...
  useEffect(() => {
    const previous = previousMediaStatusRef.current
    previousMediaStatusRef.current = mediaSession.status
    if (previous !== 'STOPPING' || mediaSession.status !== 'IDLE') return
    if (!projectRoot || !currentRecordingId || !String(mediaSession.transcriptText || '').trim()) return
    // Persist the final, stop-time refined transcript automatically. This is
    // the source for the previous-recording list; users do not need to press
    // the manual save button for every session.
    void persistLiveTextFile('TRANSCRIPT', mediaSession.transcriptText).catch((saveError) => {
      setRecordingHistoryError(`최종 Transcript 자동 저장 실패: ${String((saveError as Error)?.message || saveError)}`)
    })
  }, [mediaSession.status, currentRecordingId, projectRoot])
  ...
  const parseSummaryErrorDetail = (error: LegacyValue) => {
    let message=String((error as Error)?.message||error||'요약정리 실패')
    let logPath=''
    ...
    if(responseBody){
      try{
        ...
        if(detail&&typeof detail==='object'){
          ...
          logPath=String(detail?.log_path||'')
        }else if(typeof detail==='string'&&detail.trim()){
          message=detail
        }
        if(!logPath) logPath=String(parsed?.log_path||'')
      }catch{}
    }
  ...
  const summarizeLiveTranscript = async () => {
    ...
    setLiveSummaryFallback(false)
    try {
      const result = await api<{ summary?: string; truncated?: boolean; fallback?: boolean; warning?: string; log_path?: string }>('/media-stt/summarize', {
        method: 'POST',
      ...
      setLiveSummaryErrorLogPath(String(result?.log_path||''))
      // A generated summary belongs to the recording session. Persist it
      // immediately so previous-recording history can display it later.
      void persistLiveTextFile('SUMMARY', summary).catch(() => {})
      if(result?.fallback){
        setLiveSummaryFallback(true)
        setLiveSummaryError(String(result?.warning||'AI Provider 응답 실패로 로컬 안전 요약을 사용했습니다.'))
        setStatus('로컬 안전 요약을 생성했습니다. AI Provider 경고는 별도 영역에서 확인할 수 있습니다.')
      }else{
        setLiveSummaryFallback(false)
        setLiveSummaryError('')
      ...
      const detail=parseSummaryErrorDetail(summaryError)
      setLiveSummaryFallback(false)
      setLiveSummaryError(`요약정리 실패: ${detail.message}`)
      setLiveSummaryErrorLogPath(detail.logPath)
    } finally {
  ...
  const persistLiveTextFile = async (kind: 'TRANSCRIPT' | 'SUMMARY', text: string) => {
    ...
    const result = await api<{ path?: string; relative_path?: string }>('/media-stt/save-text', {
      ...
      body: JSON.stringify({
        ...
        started_at: mediaSession.startedAt,
        duration_seconds: mediaSession.elapsedSeconds
      })
    })
    ...
    void loadRecordingHistory(currentRecordingId)
    return savedPath
  }
  ...
  const saveLiveSummaryFile = async () => {
    ...
    try {
      ...
      const detail=parseSummaryErrorDetail(saveError)
      setLiveSummaryFallback(false)
      setLiveSummaryError(`요약정리 파일 저장 실패: ${detail.message}`)
      setLiveSummaryErrorLogPath(detail.logPath)
      setStatus(`요약정리 파일 저장 실패: ${detail.message}`)
    } finally {
      setLiveFileSaving('')
    }
  }
  ...
  const visibleMemos = useMemo(() => {
    const sorted = [...memos].sort((a: LegacyValue, b: LegacyValue) => String(b.updatedAt).localeCompare(String(a.updatedAt)))
    if (filterMode === 'CURRENT' && normalizedActiveFile) {
      return sorted.filter((item: LegacyValue) => memoFileKey(item.filePath) === memoFileKey(normalizedActiveFile))
    }
    return sorted
  ...
  const startNewMemo = () => {
    const target = normalizedActiveFile || memoTargetFiles[0] || selectableFiles.find((path: LegacyValue) => !usedFileKeys.has(memoFileKey(path))) || ''
    const existing = target ? memos.find((item: LegacyValue) => memoFileKey(item.filePath) === memoFileKey(target)) || null : null
    if (existing) {
      loadMemo(existing)
  ...
  const saveMemo = async () => {
    ...
    const conflictingMemo = memos.find((item: LegacyValue) => memoFileKey(item.filePath) === memoFileKey(targetFile) && item.id !== selectedId)
    if (conflictingMemo) {
    ...
    let id = selectedId
    if (selectedMemo) {
      next = memos.map((item: LegacyValue) => item.id === selectedMemo.id
  ...
  const deleteMemo = async () => {
    ...
    const next = memos.filter((item: LegacyValue) => item.id !== selectedMemo.id)
    try {
      await persistMemos(projectRoot, next)
      setMemos(next)
      setSelectedId('')
      setDraftFile(normalizedActiveFile || selectableFiles.find((path: LegacyValue) => !next.some((item: LegacyValue) => memoFileKey(item.filePath) === memoFileKey(path))) || '')
      setDraftTitle('')
      setDraftContent('')
  ...
  const renderLivePanel = () => (
    <div className="project-live-recording-panel">
      ...
      <div className="project-live-recording-options">
        <label>
          <span>입력</span>
          <select value={liveSourceType} onChange={(event: LegacyValue) => setLiveSourceType(event.target.value === 'SCREEN' ? 'SCREEN' : 'MICROPHONE')} disabled={mediaSession.status !== 'IDLE'}>
            <option value="MICROPHONE">마이크 / 외부 음성</option>
            <option value="SCREEN">화면 / 시스템 오디오</option>
          </select>
        </label>
        <label className="project-live-stt-toggle">
          <input type="checkbox" checked={liveEnableStt} onChange={(event: LegacyValue) => setLiveEnableStt(event.target.checked)} disabled={mediaSession.status !== 'IDLE'} />
      ...
      <div className="project-live-media-launcher">
        <input
          ...
          onChange={(event: LegacyValue) => setExternalMediaUrl(event.target.value)}
      ...
      <div className="project-live-transcript" aria-live="polite">
        <div className="project-live-transcript-head">
          <div>
            ...
            <small>녹음 중에는 가벼운 임시 STT만 표시하고, 녹음 정지 후 전체 음성을 고품질 모델로 정밀 분석해 최종 Transcript로 교체합니다.</small>
          </div>
        ...
        <div className="project-live-transcript-body">
          ...
          {mediaSession.transcriptSegments.map((segment: LegacyValue) => (
            <p key={segment.id} className={segment.refined ? 'refined' : 'collected'} title={segment.confidence == null ? '' : `인식 신뢰도 ${Math.round(segment.confidence * 100)}%`}>
              ...
              <em>{segment.refined ? '정밀 완료' : '수집됨'}</em>
            </p>
          ))}
      ...
      {liveSummaryError && (
        <div className={`project-live-summary-provider-notice ${liveSummaryFallback ? 'warning' : 'error'}`}>
          <div className="project-live-summary-provider-title">
            <strong>{liveSummaryFallback ? '⚠ AI Provider 경고' : '✕ 요약 처리 오류'}</strong>
            {liveSummaryFallback && <span>로컬 요약 생성 완료</span>}
          </div>
          <div>{liveSummaryError}</div>
          {liveSummaryErrorLogPath && <div className="project-live-summary-log-path"><strong>오류 로그</strong><code title={liveSummaryErrorLogPath}>{liveSummaryErrorLogPath}</code></div>}
        </div>
      )}

      {(liveSummary || liveSummaryLoading) && (
        <section className={`project-live-summary project-live-summary-bottom ${liveSummaryFallback ? 'fallback' : 'ai'}`}>
          <div className="project-live-summary-head">
            <div>
              <strong>{liveSummaryFallback ? 'Transcript 요약 · 로컬 안전 요약' : 'Transcript 요약 · AI'}</strong>
              <small>{liveSummarySegmentCount > 0 ? `${liveSummarySegmentCount}개 구간 기준` : '현재 Transcript 기준'}</small>
            </div>
      ...
      <section className="project-recording-history">
        <div className="project-recording-history-head">
          <div><strong>이전 녹음 기록</strong><small>최종 Transcript 파일과 저장된 요약을 다시 확인할 수 있습니다.</small></div>
          <button type="button" onClick={() => void loadRecordingHistory()} disabled={recordingHistoryLoading}>{recordingHistoryLoading ? '불러오는 중…' : '↻ 새로고침'}</button>
        </div>
        {recordingHistoryError && <div className="project-memo-status error">{recordingHistoryError}</div>}
        <div className="project-recording-history-layout">
          <div className="project-recording-history-list">
            {!recordingHistoryLoading && recordingHistory.length === 0 && <small>저장된 이전 녹음 Transcript가 없습니다.</small>}
            {recordingHistory.map((item: RecordingHistoryItem) => (
              <button type="button" key={item.recording_id} className={selectedRecordingId === item.recording_id ? 'active' : ''} onClick={() => void openRecordingHistory(item.recording_id)}>
                <span>{item.started_at ? new Date(item.started_at).toLocaleString('ko-KR') : '날짜 없음'}</span>
                <b>{formatElapsed(item.duration_seconds)}</b>
                <em className={item.has_summary ? 'has-summary' : ''}>{item.has_summary ? '요약 있음' : '요약 없음'}</em>
              </button>
            ))}
          </div>
          <div className="project-recording-history-detail">
            {!selectedRecordingId && <small>목록에서 녹음 기록을 선택하세요.</small>}
            {selectedRecordingId && <>
              <div className="history-detail-block"><strong>최종 Transcript</strong><pre>{selectedRecordingTranscript || '저장된 Transcript 파일이 없습니다.'}</pre></div>
              <div className="history-detail-block"><strong>요약</strong><pre>{selectedRecordingSummary || '저장된 요약 내용이 없습니다.'}</pre></div>
            </>}
          </div>
        </div>
      </section>

      <div className="project-live-bottom-spacer" aria-hidden="true" />
    </div>
  )
  ...
  return (
    <div className={`project-memo-panel code-tab-panel ${panelMode === 'LIVE' ? 'live-mode' : 'memo-mode'}`}>
      ...
      <div className="project-memo-workspace" ref={workspaceRef}>
        ...
        >
          ...
          {visibleMemos.map((memo: LegacyValue) => (
            <button
              type="button"
              key={memo.id}
        ...
        <div className="project-memo-editor">
          <label>
            ...
            <select
              ...
              onChange={(event: LegacyValue) => setDraftFile(event.target.value)}
              title={selectedMemo ? '저장된 메모의 파일은 변경할 수 없습니다. 파일별 메모는 1개입니다.' : '메모를 저장할 파일을 선택합니다.'}
            >
              {!draftFile && <option value="">파일 선택</option>}
              {memoTargetFiles.map((path: LegacyValue) => <option key={path} value={path}>{path}</option>)}
          ...
          <label>
            ...
            <input
              value={draftTitle}
              onChange={(event: LegacyValue) => setDraftTitle(event.target.value)}
              placeholder="메모 제목"
            />
          </label>
          <label className="project-memo-content-label">
            ...
            <textarea
              value={draftContent}
              onChange={(event: LegacyValue) => setDraftContent(event.target.value)}
              onKeyDown={(event: LegacyValue) => {
