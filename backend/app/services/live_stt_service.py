...
class RefineWhisperRuntime(FasterWhisperRuntime):
    """Separate high-quality runtime used only after recording stops.

    Live recognition stays lightweight (`AGENTSTUDIO_STT_MODEL`, default small).
    Stop-time refinement can use a larger model without forcing that model into
    the latency-sensitive live path.  GPU use remains explicit/configurable.
    """

    def config(self) -> dict[str, str]:
        model_name = str(os.getenv("AGENTSTUDIO_STT_REFINE_MODEL", "medium") or "medium").strip()
        live_device = str(os.getenv("AGENTSTUDIO_STT_DEVICE", "cpu") or "cpu").strip().lower()
        device = str(os.getenv("AGENTSTUDIO_STT_REFINE_DEVICE", live_device) or live_device).strip().lower()
        compute_type = str(
            os.getenv(
                "AGENTSTUDIO_STT_REFINE_COMPUTE_TYPE",
                "float16" if device == "cuda" else "int8",
            )
            or ("float16" if device == "cuda" else "int8")
        ).strip()
...
refine_whisper_runtime = RefineWhisperRuntime()
...
class LiveSttSession:
    ...
    async def finish(self, refine: bool = True) -> list[dict[str, Any]]:
        ...
        model = await refine_whisper_runtime.ensure_model(self._send)
        duration_ms = int(self._total_samples * 1000 / self.sample_rate)
        await self._send(
            {
                "type": "refine_status",
                "status": "RUNNING",
                "message": f"녹음 전체를 {refine_whisper_runtime.config()['model']} 모델로 다시 분석해 누락/오인식 문장을 정밀 보정하고 있습니다…",
...
def live_stt_runtime_status() -> dict[str, Any]:
    return {
        **faster_whisper_runtime.status(),
        ...
        "refine_runtime": refine_whisper_runtime.status(),
        "refine_after_stop_only": True,
    }
