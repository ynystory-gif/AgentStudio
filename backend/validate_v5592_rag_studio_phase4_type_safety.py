...
assert "AGENTSTUDIO_FRONTEND_VERSION='5.592'" in APP
assert 'version="5.592"' in MAIN
assert '"version": "5.592"' in API_ROUTES
assert 'RagStudioPhase4AgentIntegration+RagStudioPhase4StrictTypeSafetyBuildFix' in API_ROUTES


# v5.592 strict TypeScript regression guard: no unchecked direct property access from array[0].
TSCONFIG = (FRONTEND / 'tsconfig.app.json').read_text(encoding='utf-8')
assert '"noUncheckedIndexedAccess": true' in TSCONFIG
assert 'if(toolItems.length)setSelectedAgentToolId(toolItems[0].id)' not in RAG_UI
assert 'if(collectionItems.length)setToolCollectionId(collectionItems[0].id)' not in RAG_UI
for token in (
...
print('[PASS] v5.592 RAG Studio phase 4 strict TypeScript build safety + Agent integration contracts')
