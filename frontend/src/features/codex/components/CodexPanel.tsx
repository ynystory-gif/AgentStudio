...
import { AiAttachmentPicker, type AiAttachment, type AiAttachmentAnalysisState } from '../../../components/ai/AiAttachmentPicker'
import type {
  CodexModel,
...
function userInputQuestions(request: CodexServerRequest): Array<LegacyRecord> {
  const questions = request.params.questions
  return Array.isArray(questions) ? questions.filter((row: LegacyValue) => row && typeof row === 'object') as Array<LegacyRecord> : []
}
...
function transcriptFromThread(thread: LegacyValue): CodexTranscriptItem[] {
...
export function CodexPanel({ projectRoot, activeFile = '', onCodeProposal }: Props) {
  ...
  const selectedModel = useMemo(() => models.find((row: LegacyValue) => modelId(row) === model) || null, [models, model])
  ...
  const pushTranscript = useCallback((item: CodexTranscriptItem) => {
    setTranscript((prev: LegacyValue) => [...prev, item].slice(-300))
  }, [])
  ...
  const appendAssistantDelta = useCallback((delta: string) => {
    ...
    if (!existingId) {
      ...
      setTranscript((prev: LegacyValue) => [...prev, { id, kind: 'assistant', text: delta, createdAt: Date.now() }])
      return
    }
    setTranscript((prev: LegacyValue) => prev.map((item: LegacyValue) => item.id === existingId ? { ...item, text: item.text + delta } : item))
  }, [])
  ...
  const refreshStatus = useCallback(async () => {
    try {
      ...
      setStatus(next)
      setModel((current: LegacyValue) => {
        if (current || !next.models?.length) return current
        const preferred = next.models.find((row: LegacyValue) => row.isDefault) || next.models[0]
        return preferred ? modelId(preferred) : current
      })
      setThreadId((current: LegacyValue) => current || next.current_thread_id || '')
      if (next.active_turn_id) setTurnId(next.active_turn_id)
      setPendingApprovals(next.pending_requests || [])
    } catch (error) {
      if (!mountedRef.current) return
      setStatus((prev: LegacyValue) => ({ ...prev, last_error: asLegacyError(error).message || String(error) }))
    }
  }, [projectRoot])
  ...
  const startCodex = useCallback(async () => {
    ...
    try {
      ...
      if (next.models?.length) {
        const preferred = next.models.find((row: LegacyValue) => row.isDefault) || next.models[0]
        if (preferred) {
          setModel((current: LegacyValue) => current || modelId(preferred))
        }
      }
    } catch (error) {
      setStatus((prev: LegacyValue) => ({ ...prev, last_error: asLegacyError(error).message || String(error) }))
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event: LegacyValue) => {
      ...
      if (wire.type === 'codex/server-request') {
        ...
        setPendingApprovals((prev: LegacyValue) => prev.some((row: LegacyValue) => row.request_id === req.request_id) ? prev : [...prev, req])
        return
      }
      ...
      if (method === 'serverRequest/resolved') {
        ...
        setPendingApprovals((prev: LegacyValue) => prev.filter((row: LegacyValue) => row.request_id !== requestId))
        setApprovalAnswers((prev: LegacyValue) => {
          const next = { ...prev }
          Object.keys(next).filter((key: LegacyValue) => key.startsWith(`${requestId}:`)).forEach((key: LegacyValue) => delete next[key])
          return next
        })
        return
      }
      ...
      if (method === 'item/reasoning/summaryTextDelta') {
        setReasoning((prev: LegacyValue) => (prev + String(params.delta || '')).slice(-6000))
        return
      }
      if (method === 'item/commandExecution/outputDelta') {
        const itemId = String(params.itemId || '')
        const delta = String(params.delta || '')
        setTranscript((prev: LegacyValue) => prev.map((item: LegacyValue) => item.id === itemId ? { ...item, text: item.text + delta } : item))
        return
      }
        ...
        if (type === 'agentMessage' && completed) {
          ...
          if (text) {
            ...
            if (parsedProposal && onCodeProposal) {
              ...
              setTranscript((prev: LegacyValue) => {
                const filtered = activeId ? prev.filter((row: LegacyValue) => row.id !== activeId) : prev.filter((row: LegacyValue) => row.id !== id)
                return [...filtered, {
                  id: nowId('proposal'),
                  kind: 'assistant',
              ...
              const activeId = activeAssistantIdRef.current
              if (activeId) {
                setTranscript((prev: LegacyValue) => prev.map((row: LegacyValue) => row.id === activeId ? { ...row, text } : row))
              } else {
                setTranscript((prev: LegacyValue) => [...prev, { id, kind: 'assistant', text, createdAt: Date.now() }])
              }
            }
          }
        } else if (type === 'commandExecution') {
          ...
          setTranscript((prev: LegacyValue) => {
            const exists = prev.some((x: LegacyValue) => x.id === id)
            return exists ? prev.map((x: LegacyValue) => x.id === id ? row : x) : [...prev, row]
          })
        } else if (type === 'fileChange') {
          const row: CodexTranscriptItem = {
            id,
  ...
  async function loginChatGPT() {
    ...
    setBusy(true)
    try {
      const result = await api<LegacyValue>('/codex/login/chatgpt', { method: 'POST', body: JSON.stringify({ root: projectRoot }) })
  ...
  async function createThread(): Promise<string> {
    const result = await api<{ thread: CodexThread }>('/codex/thread/start', {
      method: 'POST',
  ...
  async function resumeThread(thread: CodexThread) {
    ...
    setBusy(true)
    try {
      const result = await api<{ thread: CodexThread }>('/codex/thread/resume', {
        method: 'POST',
      ...
      pushTranscript({ id: nowId('error'), kind: 'error', text: asLegacyError(error).message || String(error), createdAt: Date.now() })
    } finally {
      setBusy(false)
    }
  }
  ...
  async function sendMessage() {
    ...
    const attachmentLabel = attachments.length
      ? `\n\n📎 참고 파일: ${attachments.map((item: LegacyValue) => item.name).join(', ')}`
      : ''
    ...
    try {
      ...
      const result = await api<{ turn: LegacyRecord; attachment_warnings?: string[] }>('/codex/turn/start', {
        method: 'POST',
        body: JSON.stringify({
          ...
          model,
          effort,
          attachment_ids: attachments.map((item: LegacyValue) => item.attachment_id),
        })
      })
  ...
  async function resolveApproval(request: CodexServerRequest, decision: string) {
    try {
      ...
      setPendingApprovals((prev: LegacyValue) => prev.filter((row: LegacyValue) => row.request_id !== request.request_id))
      setApprovalAnswers((prev: LegacyValue) => {
        const next = { ...prev }
        Object.keys(next).filter((key: LegacyValue) => key.startsWith(`${request.request_id}:`)).forEach((key: LegacyValue) => delete next[key])
        return next
      })
    } catch (error) {
      pushTranscript({ id: nowId('error'), kind: 'error', text: asLegacyError(error).message || String(error), createdAt: Date.now() })
  ...
  async function refreshCodexUsage(force: LegacyValue = true) {
    if (!loggedIn || usageRefreshing) return
    setUsageRefreshing(true)
    try {
      const result = await api<LegacyValue>(`/codex/rate-limits?force=${force ? 'true' : 'false'}`)
      if (!result?.ok && !result?.rate_limits) {
        throw new Error(result?.message || result?.error || 'Codex 사용량을 확인할 수 없습니다.')
      }
      await refreshStatus()
    } catch (error) {
      setStatus((prev: LegacyValue) => ({ ...prev, rate_limits_error: asLegacyError(error).message || String(error) }))
    } finally {
  ...
  return <div className="codex-panel code-tab-panel">
    <div className="codex-toolbar">
      ...
      <div className="codex-toolbar-actions">
        ...
        <div className="codex-thread-menu-wrap">
          <button type="button" title="최근 대화" onClick={() => { setDetailOpen(false); setThreadMenuOpen((v: LegacyValue) => !v); loadThreads() }}>⌄</button>
          {threadMenuOpen && <div className="codex-thread-menu">
            <div className="codex-thread-menu-head">최근 대화</div>
            {threads.length ? threads.map((row: LegacyValue) => <button type="button" key={row.id} onClick={() => resumeThread(row)}>
    ...
    {status.initialized && (loggedIn || status.requires_openai_auth === false) && <>
      ...
      <div className="codex-transcript" ref={transcriptRef}>
        ...
        {transcript.map((item: LegacyValue) => <div className={`codex-message ${item.kind}`} key={item.id}>
        ...
        {pendingApprovals.map((request: LegacyValue) => {
          ...
          return <div className="codex-approval-card" key={request.request_id}>
            ...
            {isUserInput ? <div className="codex-user-input-questions">
              {questions.map((question: LegacyValue, questionIndex: LegacyValue) => {
                ...
                return <div className="codex-user-input-question" key={questionId || `question-${questionIndex}`}>
                  ...
                  {!!options.length && <div className="codex-user-input-options">
                    {options.map((option: LegacyValue) => <button
                      type="button"
                      key={String(option.label || '')}
                      className={answer === String(option.label || '') ? 'active' : ''}
                      onClick={() => setApprovalAnswers((prev: LegacyValue) => ({ ...prev, [answerKey]: String(option.label || '') }))}
                      title={String(option.description || '')}
                    >{String(option.label || '')}</button>)}
                  </div>}
                  {(question.isOther !== false || !options.length) && <input
                    type={question.isSecret ? 'password' : 'text'}
                    value={options.some((option: LegacyValue) => String(option.label || '') === answer) ? '' : answer}
                    onChange={(event: React.ChangeEvent<HTMLInputElement>) => setApprovalAnswers((prev: LegacyValue) => ({ ...prev, [answerKey]: event.target.value }))}
        ...
        <div className="codex-composer-footer">
          <div className="codex-selectors">
            <select value={model} onChange={(event: React.ChangeEvent<HTMLSelectElement>) => setModel(event.target.value)} title="Codex 모델">
              ...
              {models.map((row: LegacyValue) => <option key={modelId(row)} value={modelId(row)}>{modelLabel(row)}</option>)}
            </select>
            <select value={effort} onChange={(event: React.ChangeEvent<HTMLSelectElement>) => setEffort(event.target.value)} title="Reasoning effort">
              {efforts.map((value: LegacyValue) => <option value={value} key={value}>{value}</option>)}
            </select>
          </div>
          {turnId
