...
export function LearningProblemProgressEnhancer(){
  useEffect(()=>{
    let stopped=false
    let busy=false
    const findProblemBlock=()=>[...document.querySelectorAll('.llm-job-progress')].find(el=>String(el.textContent||'').includes('문제 수집'))
    const getJobId=(root)=>{
      const text=String(root?.textContent||'')
      const match=text.match(/작업 ID:\s*([a-f0-9]{16,})/i)
      return match?.[1]||''
    }
    const render=(root,job)=>{
      ...
      panel.innerHTML=items.map(([label,value])=>`<div style="min-width:0"><span style="display:block;color:#8997aa;margin-bottom:3px">${label}</span><b style="display:block;color:#e7edf5;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${String(value)}</b></div>`).join('')
