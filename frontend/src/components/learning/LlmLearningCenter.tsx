  ...
  return <>
    ...
    {open&&<div className="llm-learning-overlay" role="dialog" aria-modal="true">
      <div className="llm-learning-window">
        ...
        <div className="llm-learning-shared-db-note"><b>🌐 공용 Runtime DB</b><span>A PC · B PC 등 같은 AgentStudio DB를 사용하는 모든 PC가 동일한 오판 사례와 Dataset을 조회·검토·이어 작업합니다.</span><em>현재 PC: {summary.current_pc_name||'-'}</em></div>
        ...
        <main>
          ...
          {tab==='datasets'&&<table><thead><tr><th>상태</th><th>생성 PC</th><th>학습 범위</th><th>문제 수</th><th>검증</th><th>작업</th></tr></thead><tbody>{datasets.map(row=><tr key={row.id}><td>{row.status}</td><td>{row.source_pc_name||'-'}<small>{row.updated_by_pc_name&&row.updated_by_pc_name!==row.source_pc_name?`최근 수정: ${row.updated_by_pc_name}`:''}</small></td><td><b>{row.scope?.domain||'-'} / {row.scope?.topic||'-'}</b><small>{row.scope?.learning_objective||''}</small></td><td>{row.problem_count||0}</td><td>{row.validation?.approved||0} 승인 / {row.validation?.pending||0} 대기</td><td>{row.status==='review'&&<button onClick={()=>validate(row)}>전체 검증 완료</button>}{row.status==='validated'&&<button onClick={()=>prepare(row)}>이 PC에서 학습 준비</button>}</td></tr>)}</tbody></table>}
          {tab==='training'&&<div className="llm-learning-training"><h3>학습 적용 안전 Gate</h3><p>① 공용 DB 검증 Dataset → ② Train/Validation/Test 분리 → ③ LoRA/QLoRA 학습 → ④ 기존 모델과 Blind Test 비교 → ⑤ 최소 향상폭 통과 → ⑥ Ollama 등록 순서입니다.</p><p><b>현재 Base Model:</b> {summary.current_ollama_model||'-'} · <b>라우팅:</b> {summary.current_strategy||'-'}</p><div className="llm-learning-gate">오판·문제·검증·평가 상태는 모든 PC가 공유합니다. QLoRA Adapter 같은 대용량 학습 산출물만 학습을 실행한 PC 로컬에 유지됩니다.</div>{datasets.filter(x=>x.status==='training'||x.status==='trained'||x.status==='deployed').map(row=><article key={row.id}><b>{row.scope?.topic||row.id}</b><span>{row.status} · Train {row.split?.train||0} / Validation {row.split?.validation||0} / Test {row.split?.test||0}</span><small>{row.training?.prepared_by_pc_name?`학습 준비 PC: ${row.training.prepared_by_pc_name} · `:''}{row.training?.dataset_dir||''}</small></article>)}</div>}
        </main>
      </div>
    </div>}
  </>
