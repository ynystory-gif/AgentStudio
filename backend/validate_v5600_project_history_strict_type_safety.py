...
# Version contract.
require('version="5.600"' in main_py, 'Backend FastAPI version is not 5.600')
require("AGENTSTUDIO_FRONTEND_VERSION='5.600'" in app_tsx, 'Frontend version is not 5.600')
require('"version": "5.600"' in routes_py and 'AccountProjectSettingsDbHistory' in routes_py and 'ProjectHistoryStrictTypeSafetyBuildFix' in routes_py, 'Health/build marker is missing')
...
# Project History strict-safe first-item access regression guard.
require('next[0].id' not in history_tsx, 'Unsafe next[0].id access reintroduced in ProjectHistoryPanel')
require('const firstItem=next[0]' in history_tsx, 'Strict-safe firstItem guard is missing in ProjectHistoryPanel')
require('if(firstItem&&!selectedId)setSelectedId(firstItem.id)' in history_tsx, 'firstItem existence check is missing')
...
print('[PASS] v5.600 account/project settings + project history strict type safety contracts')
