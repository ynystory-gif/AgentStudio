...
const px=(value: LegacyValue)=>typeof value==='number'?`${value}px`:String(value||'')
const apply=(el: LegacyValue,rule:LegacyRecord={})=>{
...
function visible(el: LegacyValue){
  if(!el)return false
  const s=getComputedStyle(el)
  return s.display!=='none'&&s.visibility!=='hidden'&&el.getBoundingClientRect().width>0
}

function activeTheme(themes: LegacyValue){
  const selects=[...document.querySelectorAll('select')].filter(visible)
  for(const select of selects){
    const opt=select.options?.[select.selectedIndex]
    const label=String(opt?.textContent||'').trim()
    const value=String(select.value||'').trim()
    const found=themes.find((t: LegacyValue)=>String(t.id)===value||String(t.name||'').trim()===label||label.includes(String(t.name||'').trim()))
    if(found)return found
  }
  return null
}

function previewMode(root: LegacyValue){
...
function layoutContract(layout:LegacyRecord={}){
...
function clearMobileStructure(root: LegacyValue,sidebarRoot: LegacyValue){
  if(!root)return
  ...
  ;['position','top','bottom','left','right','zIndex','maxWidth','height','boxShadow','display'].forEach((key: LegacyValue)=>{sidebarRoot.style[key]=''})
}

const navGlyph=(label: LegacyValue='',index: LegacyValue=0)=>{
...
function sourceNavigation(sidebarRoot: LegacyValue,items: LegacyValue,itemDetails: LegacyValue,presentation: LegacyValue,menuNormal: LegacyValue,menuActive: LegacyValue){
  ...
  slots.forEach((item: LegacyValue,index: LegacyValue)=>{
...
function applyMobileDrawer(root: LegacyValue,sidebarRoot: LegacyValue,contract: LegacyValue,rules: LegacyValue,layout: LegacyValue){
...
function styleWireframe(root: LegacyValue,theme: LegacyValue){
  ...
  navItems.forEach((item: LegacyValue,index: LegacyValue)=>{
  ...
  ;[...root.querySelectorAll('.ui-layout-wf-sidebar i')].forEach((item: LegacyValue,index: LegacyValue)=>{
  ...
  ;[...root.querySelectorAll('.ui-layout-wf-main > div:not(.ui-layout-wf-search), .ui-layout-wf-card')].forEach((el: LegacyValue)=>apply(el,card.normal||card))
  ;[...root.querySelectorAll('.ui-layout-wf-search')].forEach((el: LegacyValue)=>apply(el,input.normal||input))
  ;[...root.querySelectorAll('.ui-layout-wf-header b, .ui-layout-wf-main b')].forEach((el: LegacyValue)=>apply(el,button.normal||button))
...
export function ImportedThemePreviewEnhancer(){
  useEffect(()=>{
    ...
    const scan=()=>{
      ...
      if(!theme)return
      document.querySelectorAll('.ui-layout-wireframe').forEach((root: LegacyValue)=>styleWireframe(root,theme))
    }
