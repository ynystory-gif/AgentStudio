...
export function CodexSettingsPanel({ enabled, busy = false, onEnabledChange, onSave }: Props) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setError(asLegacyError(e).message || String(e))
      return null
    }
  }, [])
  ...
  async function start() {
    ...
    } catch (e) { setError(asLegacyError(e).message || String(e)) }
  ...
  async function login() {
    ...
    try {
      await onSave()
      const result = await api<LegacyValue>('/codex/login/chatgpt', { method: 'POST', body: JSON.stringify({ root: '' }) })
  ...
  async function refreshUsage() {
    setLocalBusy(true); setMessage(''); setError('')
    try {
      const result = await api<LegacyValue>('/codex/rate-limits?force=true')
  ...
  return <section className="settings-panel codex-settings-panel">
    ...
    {!!usageBuckets.length && <div className="codex-usage-settings">
      ...
      {usageBuckets.map((bucket: CodexRateLimitSnapshot, index: LegacyValue) => <div className="codex-usage-bucket" key={String(bucket.limitId || bucket.limitName || index)}>
