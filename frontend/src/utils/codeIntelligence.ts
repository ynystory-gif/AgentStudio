...
function trimCache(): void {
  ...
  Array.from(REQUEST_CACHE.keys()).slice(0, remove).forEach((key: LegacyValue) => REQUEST_CACHE.delete(key))
}

function contextForModel(model: LegacyValue): RegisteredContext | undefined {
  return MODEL_CONTEXTS.get(String(model?.uri?.toString?.() || ''))
}

function modelWordRange(monaco: LegacyValue, model: LegacyValue, position: LegacyValue) {
  const word = model?.getWordAtPosition?.(position)
  if (!word) return undefined
  return new monaco.Range(position.lineNumber, word.startColumn, position.lineNumber, word.endColumn)
}

async function resolveAt(model: LegacyValue, position: LegacyValue, action: LegacyValue = 'all'): Promise<CodeIntelligenceResult> {
...
function targetUri(monaco: LegacyValue, context: RegisteredContext, definition: DefinitionTarget) {
...
function ensureTargetModel(monaco: LegacyValue, context: RegisteredContext, definition: DefinitionTarget) {
...
function installOpener(monaco: LegacyValue): void {
  if (OPENER_MONACO.has(monaco)) return
  OPENER_MONACO.add(monaco)
  monaco.editor.registerEditorOpener?.({
    openCodeEditor: async (source: LegacyValue, resource: LegacyValue, selection: LegacyValue) => {
...
function installProviders(monaco: LegacyValue): void {
  if (PROVIDER_MONACO.has(monaco)) return
  PROVIDER_MONACO.add(monaco)
  installOpener(monaco)

  PROVIDER_LANGUAGES.forEach((language: LegacyValue) => {
    monaco.languages.registerDefinitionProvider(language, {
      provideDefinition: async (model: LegacyValue, position: LegacyValue) => {
        const context = contextForModel(model)
    ...
    monaco.languages.registerHoverProvider(language, {
      provideHover: async (model: LegacyValue, position: LegacyValue) => {
    ...
    monaco.languages.registerCompletionItemProvider(language, {
      triggerCharacters: ['.'],
      provideCompletionItems: async (model: LegacyValue, position: LegacyValue) => {
        ...
        return {
          suggestions: completions.map((item: LegacyValue, index: number) => ({
  ...
  monaco.languages.registerSignatureHelpProvider('python', {
    ...
    provideSignatureHelp: async (model: LegacyValue, position: LegacyValue) => {
      if (!contextForModel(model)) return null
      ...
      return {
        value: {
          signatures: [{
            ...
            parameters: params.map((parameter: LegacyValue) => ({
...
export function registerCodeIntelligence(monaco: LegacyValue, editor: LegacyValue, context: CodeIntelligenceContext): { dispose: () => void } {
  ...
  const keyDisposable = editor.onKeyDown?.((event: LegacyValue) => {
  ...
  const mouseDisposable = editor.onMouseDown?.(async (event: LegacyValue) => {
