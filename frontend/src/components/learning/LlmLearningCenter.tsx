...
export function LlmLearningCenter() {
  ...
  return <>
    <button className={`llm-learning-nav-button ${open?'active':''}`} type="button" onClick={()=>setOpen(v=>!v)} title="학습" aria-label="학습">
      <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9.5 4.5a3 3 0 0 0-5 2.2 3.1 3.1 0 0 0 .6 1.8A3.5 3.5 0 0 0 4 11a3.5 3.5 0 0 0 2 3.2V16a3 3 0 0 0 3.5 3V4.5Zm5 0a3 3 0 0 1 5 2.2 3.1 3.1 0 0 1-.6 1.8A3.5 3.5 0 0 1 20 11a3.5 3.5 0 0 1-2 3.2V16a3 3 0 0 1-3.5 3V4.5ZM9.5 9H7.8M14.5 9h1.7M9.5 14H8m6.5 0H16"/></svg>
    </button>
    {open&&<section className="llm-learning-workspace" aria-label="LLM 학습 센터">
      <div className="llm-learning-window">
        <header><div><strong>LLM 학습 센터</strong><small>오판 수집 → 관련 문제 대량 생성 → 검증 → 학습/평가 → PC별 Ollama 적용</small></div><button className="llm-learning-back" onClick={()=>setOpen(false)}>본문으로 돌아가기</button></header>
        <div className="llm-learning-shared-db-note"><b>🌐 공용 학습 데이터</b><span>오판 사례·생성 문제·Dataset·검증·평가 결과는 등록된 여러 PC에서 동일하게 조회합니다. 학습 모델 설치·사용 여부만 PC별로 따로 관리합니다.</span><em>현재 PC: {summary.current_pc_name||'-'}</em></div>
