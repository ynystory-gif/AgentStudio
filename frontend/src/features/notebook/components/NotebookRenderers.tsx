...
function notebookAttachmentDataUrl(attachments: NotebookAttachments, name: string): string {
  ...
  const mime = preferred.find((type: LegacyValue) => item[type] !== undefined)
    || Object.keys(item).find((type: LegacyValue) => String(type).startsWith('image/'))
...
export function NotebookMarkdown({ text, attachments = {} }: NotebookMarkdownProps) {
  ...
  while (i < lines.length) {
    ...
    if (trimmed.includes('|') && /^\s*\|?\s*:?-{3,}:?\s*(\|\s*:?-{3,}:?\s*)+\|?\s*$/.test(lines[i + 1] || '')) {
      const splitRow = (line: string): string[] => line.trim().replace(/^\||\|$/g, '').split('|').map((cell: LegacyValue) => cell.trim())
      ...
      blocks.push(
        <div className="notebook-markdown-table-wrap" key={`md-${blockKey++}`}>
          <table className="notebook-markdown-table">
            <thead>
              <tr>
                {headers.map((cell: LegacyValue, idx: LegacyValue) => (
                  <th key={idx}>{renderNotebookInline(cell, `th-${blockKey}-${idx}`, attachments)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((row: LegacyValue, rowIndex: LegacyValue) => (
                <tr key={rowIndex}>
                  {headers.map((_: LegacyValue, cellIndex: LegacyValue) => (
                    <td key={cellIndex}>
    ...
    if (/^>\s?/.test(trimmed)) {
      ...
      blocks.push(
        <blockquote key={`md-${blockKey++}`}>
          {quote.map((line: LegacyValue, idx: LegacyValue) => (
    ...
    if (/^[-*+]\s+/.test(trimmed)) {
      ...
      blocks.push(
        <ul key={`md-${blockKey++}`}>
          {items.map((item: LegacyValue, idx: LegacyValue) => (
...
function notebookWarningInfo(text: string): { count: number; types: string[] } | null {
  ...
  const types = Array.from(new Set(matches.map((match: LegacyValue) => String(match[0] || '').trim()).filter(Boolean)))
