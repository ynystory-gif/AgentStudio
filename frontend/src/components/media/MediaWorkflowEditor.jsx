...
const mediaExtension=(workflow)=>workflow?.extensions?.media&&typeof workflow.extensions.media==='object'?workflow.extensions.media:{}
const editorShape=(workflow)=>{
...
const nodeId=(type,index)=>`${String(type||'MEDIA').toLowerCase()}_${Date.now().toString(36)}_${index}`
const findDef=(catalog,type)=>catalog.find(row=>row.type===type)||FALLBACK_CATALOG.find(row=>row.type===type)||null
const firstPort=(def,key,type)=>((def?.[key]||[]).find(port=>!type||port.type===type)||(def?.[key]||[])[0]||null)
const makeNode=(type,x,y,index,catalog)=>({
...
const makeEdge=(source,target,output,input,index)=>({
...
function seedWorkflow(workflow,catalog){
  const value=editorShape(workflow)
  if(value.nodes.length) return value
  const text=[value.name,...(value.steps||[]).flatMap(step=>[step?.name,step?.label,step?.description])].join(' ').toLowerCase()
  ...
  const d=type=>findDef(catalog,type)
  value.edges=[
...
const downloadableJson=(value,filename)=>{
...
export function MediaWorkflowEditor({workflow,onChange,onClose}){
  ...
  useEffect(()=>{
    let cancelled=false
    Promise.allSettled([loadMediaWorkflowCatalog(),loadMediaWorkflowContracts()]).then(results=>{
      if(cancelled) return
  ...
  const nodeMap=useMemo(()=>Object.fromEntries(editor.nodes.map(node=>[node.id,node])),[editor.nodes])
  ...
  const jobs=Object.values(mediaExtension(editor).jobs||{})
  const currentJob=jobs.length?jobs[jobs.length-1]:null
  const artifacts=Object.values(mediaExtension(editor).assets||{})
  const currentArtifact=artifacts.length?artifacts[artifacts.length-1]:null

  const updateNode=(id,patch)=>setEditor(prev=>({...prev,nodes:prev.nodes.map(node=>node.id===id?{...node,...patch}:node)}))
  const updateNodeConfig=(key,value)=>{
    if(!selectedNode) return
    setEditor(prev=>({...prev,nodes:prev.nodes.map(node=>node.id===selectedNode.id?{...node,config:{...(node.config||{}),[key]:value}}:node)}))
  }
  const removeNode=(id)=>{
    setEditor(prev=>({...prev,nodes:prev.nodes.filter(node=>node.id!==id),edges:prev.edges.filter(edge=>edge.source!==id&&edge.target!==id)}))
    if(selectedNodeId===id) setSelectedNodeId('')
    setMessage('Node와 연결선을 제거했습니다.')
  }
  const addNode=(type,x=80,y=80)=>{
    const node=makeNode(type,x,y,editor.nodes.length+1,catalog)
    setEditor(prev=>({...prev,nodes:[...prev.nodes,node]}))
  ...
  const connectToInput=async(targetNode,inputPort)=>{
    ...
    try{
      ...
      const duplicate=editor.edges.some(edge=>edge.source===pendingConnection.nodeId&&edge.target===targetNode.id&&edge.output_port===pendingConnection.port.name&&edge.input_port===inputPort.name)
      ...
      setEditor(prev=>({...prev,edges:[...prev.edges,edge]}))
      setPendingConnection(null)
      setMessage(`연결됨: ${pendingConnection.port.type} → ${inputPort.type}`)
    }catch(error){
      setMessage(`Port 검증 실패: ${String(error?.message||error)}`)
    }finally{setBusy('')}
  }
  ...
  const validateGraph=async()=>{
    ...
    try{
      ...
      setValidation({valid:false,issues:[{message:String(error?.message||error)}]})
      setMessage(`Workflow 검증 실패: ${String(error?.message||error)}`)
      return null
    }finally{setBusy('')}
  }
  ...
  const saveGraph=async()=>{
    ...
    try{
      ...
      setMessage(`설계 저장 실패: ${String(error?.message||error)}`)
    }finally{setBusy('')}
  }
  ...
  const importJson=async(event)=>{
    ...
    }catch(error){setMessage(`Workflow JSON 가져오기 실패: ${String(error?.message||error)}`)}
  }
  ...
  return <section className="media-workflow-editor-shell">
    ...
    <div className="media-workflow-editor-grid">
      <aside className="media-node-palette">
        ...
        <div className="media-node-palette-scroll">
          {groups.map(([category,rows])=><div className="media-palette-group" key={category}>
            <div><span>{CATEGORY_META[category]?.icon||'◇'}</span><strong>{CATEGORY_META[category]?.label||category}</strong><small>{rows.length}</small></div>
            {rows.map(row=><button
              key={row.type}
              type="button"
              draggable
              onDragStart={event=>beginDrag(event,{kind:'new',type:row.type})}
          ...
          <div className="media-workflow-canvas" style={{width:CANVAS_WIDTH,height:CANVAS_HEIGHT}}>
            ...
            {editor.nodes.map(node=>{
              ...
              return <article
                ...
                draggable
                onDragStart={event=>beginDrag(event,{kind:'move',id:node.id})}
                onClick={event=>{event.stopPropagation();setSelectedNodeId(node.id)}}
              >
                <div className="media-canvas-node-title"><span>{CATEGORY_META[def.category]?.icon||'◇'}</span><div><strong>{node.label||def.label||node.type}</strong><small>{node.type}</small></div><button type="button" title="Node 삭제" onClick={event=>{event.stopPropagation();removeNode(node.id)}}>×</button></div>
                <div className="media-canvas-node-ports inputs">
                  {(def.inputs||[]).map(port=><button key={`${port.name}-${port.type}`} type="button" className="media-port input" title={`${port.name}: ${port.type}`} onClick={event=>{event.stopPropagation();void connectToInput(node,port)}}><i></i><span>{port.name}</span><em>{port.type}</em></button>)}
                </div>
                <div className="media-canvas-node-ports outputs">
                  {(def.outputs||[]).map(port=><button key={`${port.name}-${port.type}`} type="button" className={`media-port output ${pendingConnection?.nodeId===node.id&&pendingConnection?.port?.name===port.name?'active':''}`} title={`${port.name}: ${port.type}`} onClick={event=>{event.stopPropagation();setPendingConnection({nodeId:node.id,port});setMessage(`${port.type} Output 선택됨. 연결할 Input Port를 선택하세요.`)}}><em>{port.type}</em><span>{port.name}</span><i></i></button>)}
                </div>
              </article>
            })}
          </div>
      ...
      <aside className="media-node-config-pane">
        ...
        {selectedNode&&<div className="media-config-form">
          ...
          <label><span>Provider</span><select value={selectedNode.config?.provider||'AUTO'} onChange={event=>updateNodeConfig('provider',event.target.value)}>{PROVIDERS.map(provider=><option key={provider}>{provider}</option>)}</select></label>
          <label><span>Model / Workflow</span><input value={selectedNode.config?.model||''} placeholder="Provider 기본값 또는 모델/Workflow ID" onChange={event=>updateNodeConfig('model',event.target.value)}/></label>
          <div className="media-config-row"><label><span>Width</span><input type="number" min="64" step="64" value={selectedNode.config?.width||1024} onChange={event=>updateNodeConfig('width',Number(event.target.value||0))}/></label><label><span>Height</span><input type="number" min="64" step="64" value={selectedNode.config?.height||1024} onChange={event=>updateNodeConfig('height',Number(event.target.value||0))}/></label></div>
          <label><span>Max Retry</span><input type="number" min="0" max="10" value={selectedNode.config?.max_retry??2} onChange={event=>updateNodeConfig('max_retry',Number(event.target.value||0))}/></label>
          <label><span>Prompt / Instructions</span><textarea value={selectedNode.config?.prompt||''} placeholder="필요한 경우 Node별 지시를 입력합니다." onChange={event=>updateNodeConfig('prompt',event.target.value)}/></label>
    ...
    <div className="media-workflow-bottom-grid">
      ...
      <section className={`media-workflow-validation ${validation?.valid===false?'invalid':validation?.valid===true?'valid':''}`}>
        ...
        <div className="media-validation-body"><strong>{message}</strong>{(validation?.issues||[]).slice(0,5).map((issue,index)=><span key={index}>! {issue?.message||issue?.reason||JSON.stringify(issue)}</span>)}{contracts&&<small>Backend Contract 연결됨 · Provider Adapter {contracts?.provider_adapter?.methods?.length||contracts?.provider?.methods?.length||6} methods</small>}</div>
      </section>
    </div>
  </section>
}
