...
const stageLabel=(stage: LegacyValue)=>STAGE_LABELS[String(stage||'').toLowerCase()]||String(stage||'-')


export function LearningWeightFinetuneEnhancer(){
  const [toolbar,setToolbar]=useState<LegacyValue|null>(null)
  const [panelHost,setPanelHost]=useState<LegacyValue|null>(null)
  const [capability,setCapability]=useState<LegacyValue|null>(null)
  const [job,setJob]=useState<LegacyValue|null>(null)
  ...
  useEffect(()=>{
    ...
    const locate=()=>{
      ...
      if(modelBlock instanceof HTMLElement){
        let host=document.querySelector<HTMLElement>('[data-weight-finetune-progress-host]')
  ...
  useEffect(()=>{
    ...
    api('/learning/weight-finetune/capability')
      .then((value: LegacyValue)=>{if(!cancelled)setCapability(value)})
      .catch((e: LegacyValue)=>{if(!cancelled)setError(String(e))})
  ...
  useEffect(()=>{
    ...
    const guard=()=>{
      document.querySelectorAll('.llm-learning-page button').forEach((button: LegacyValue)=>{
