...
const normalizeName=(value: LegacyValue='')=>String(value||'').trim().toLowerCase().replace(/\s+/g,' ')

const featureImpact=(name: LegacyValue='')=>{
  const text=normalizeName(name)
  const areas=new Set(['Agent Workflow','테스트 시나리오'])
  if(/회원|로그인|인증|권한|role|permission|rbac/.test(text)){
    ;['Auth/RBAC','사용자 UI','API','DB','권한별 테스트 계정'].forEach((v: LegacyValue)=>areas.add(v))
  }
  if(/상품|검색|추천|재고|catalog|product|search/.test(text)){
    ;['사용자 UI','관리자 UI','API','DB','검색/Vector/Cache','테스트 데이터'].forEach((v: LegacyValue)=>areas.add(v))
  }
  if(/주문|장바구니|결제|order|cart|payment/.test(text)){
    ;['사용자 UI','API','DB','주문 Workflow','테스트 주문 데이터'].forEach((v: LegacyValue)=>areas.add(v))
  }
  if(/rag|문서|파일|지식|embedding|벡터/.test(text)){
    ;['RAG/pgvector','Ingestion','API','DB','테스트 문서/Chunk'].forEach((v: LegacyValue)=>areas.add(v))
  }
  if(/관리자|admin/.test(text)){
    ;['관리자 UI','권한','API'].forEach((v: LegacyValue)=>areas.add(v))
  }
  if(areas.size<=2){
    ;['UI','API','관련 DB/설정'].forEach((v: LegacyValue)=>areas.add(v))
  }
  return Array.from(areas)
}

interface AgentDesignProjectToolbarProps {
  ...
  onNew?: ()=>void
  onSave?: ()=>void|Promise<LegacyValue>
  onLoad?: (project:LegacyValue)=>void|Promise<void>
  showNew?: boolean
  panelMode?: boolean
}
...
export function AgentDesignProjectToolbar({
  ...
  showNew=true,
  panelMode=false,
}:AgentDesignProjectToolbarProps){
  const [listOpen,setListOpen]=useState(false)
  const [loading,setLoading]=useState(false)
  const [rows,setRows]=useState<LegacyValue[]>([])
  ...
  const loadList=async()=>{
    ...
    try{
      ...
      setRows([])
      setError(String(asLegacyError(e).message||e))
    }finally{
      setLoading(false)
    }
  }
  ...
  return <>
    ...
    {listOpen&&<div className="agent-design-modal-backdrop" onMouseDown={()=>setListOpen(false)}>
      <div className="agent-design-project-modal" onMouseDown={(e: LegacyValue)=>e.stopPropagation()}>
        ...
        <div className="agent-design-project-list">
          {rows.map((row: LegacyValue)=><button
            ...
            onClick={async()=>{
              ...
              }catch(e){ setError(String(asLegacyError(e).message||e)) }
            }}
          >
...
export function AgentFeatureManager({detectedFeatures=[],features=[],onChange,panelMode=false}:LegacyRecord){
  const [editing,setEditing]=useState<LegacyValue|null>(null)
  ...
  const openModify=(item: LegacyValue)=>{
  ...
  const changeStatus=async(item: LegacyValue,action: LegacyValue)=>{
  ...
  return <div className={`agent-feature-manager ${panelMode?'panel-mode':''}`}>
    ...
    <div className="agent-feature-list">
      {merged.map((item: LegacyValue)=><div key={item.id||normalizeName(item.name)} className={`agent-feature-row status-${String(item.status||'ACTIVE').toLowerCase()}`}>
    ...
    {editing&&<div className="agent-feature-editor">
      ...
      <label><span>기능명</span><input value={name} onChange={(e: LegacyValue)=>setName(e.target.value)} placeholder="예: 쿠폰 / 회원등급 / 예약 변경"/></label>
      <label><span>설명</span><textarea value={description} onChange={(e: LegacyValue)=>setDescription(e.target.value)} placeholder="어떤 동작이 필요한지 간단히 입력하세요. 저장 후 Agent 설계 인터뷰에서 필요한 추가 질문을 이어서 진행합니다."/></label>
