...
function stackTables(tables: DatabaseDiagramTable[], x: number): PositionedTable[] {
  let y = PADDING_Y
  return tables.map((table: LegacyValue) => {
...
function buildSchemaLayout(document: DatabaseDiagramDocument) {
  const tables = [...document.tables].sort((a: LegacyValue, b: LegacyValue) => a.name.localeCompare(b.name))
...
function buildLayout(document: DatabaseDiagramDocument) {
  ...
  const root = document.tables.find((table: LegacyValue) => table.id === document.root_table) || document.tables[0]
  ...
  const others = document.tables.filter((table: LegacyValue) => table.id !== root.id)
  const left = others.filter((table: LegacyValue) => leftIds.has(table.id))
  const right = others.filter((table: LegacyValue) => rightIds.has(table.id) && !leftIds.has(table.id))
  const unassigned = others.filter((table: LegacyValue) => !leftIds.has(table.id) && !rightIds.has(table.id))
  unassigned.forEach((table: LegacyValue, index: LegacyValue) => (index % 2 ? right : left).push(table))
  ...
  const maxBottom = Math.max(...tables.map((table: LegacyValue) => table.y + table.height), 320)
...
function columnAnchor(table: PositionedTable, columnName: string | undefined, side: 'left' | 'right') {
  const columnIndex = Math.max(0, table.columns.findIndex((column: LegacyValue) => column.name === columnName))
...
function relationshipPath(
  ...
  const uniqueXs = [...new Set([...tableMap.values()].map((table: LegacyValue) => table.x))].sort((a: LegacyValue, b: LegacyValue) => a - b)
...
export function DatabaseDiagramViewer({ value, filePath = '' }: DatabaseDiagramViewerProps) {
  ...
  const tableMap = useMemo(() => new Map(layout.tables.map((table: LegacyValue) => [table.id, table])), [layout.tables])
  ...
  const exportPng = async () => {
    ...
    try {
      ...
      try {
        ...
        await new Promise<void>((resolve: LegacyValue, reject: LegacyValue) => {
        ...
        const pngBlob = await new Promise<Blob>((resolve: LegacyValue, reject: LegacyValue) => {
          canvas.toBlob((result: LegacyValue) => result ? resolve(result) : reject(new Error('PNG Blob 생성에 실패했습니다.')), 'image/png')
        })
  ...
  return <div className="database-diagram-viewer">
    <div className="database-diagram-toolbar">
      ...
      <div className="database-diagram-actions">
        ...
        <div className="database-diagram-zoom" aria-label="다이어그램 확대 축소">
          <button type="button" onClick={() => setZoom((value: LegacyValue) => Math.max(0.5, Math.round((value - 0.1) * 10) / 10))}>−</button>
          <button type="button" className="zoom-value" onClick={() => setZoom(1)} title="100%로 복원">{Math.round(zoom * 100)}%</button>
          <button type="button" onClick={() => setZoom((value: LegacyValue) => Math.min(2, Math.round((value + 0.1) * 10) / 10))}>＋</button>
        </div>
    ...
    >
      ...
      >
        ...
        {layout.tables.map((table: LegacyValue) => {
          ...
          return <g key={table.id}>
            ...
            {(table.columns || []).map((column: LegacyValue, index: LegacyValue) => {
        ...
        {(diagram.relationships || []).map((relationship: LegacyValue, index: LegacyValue) => {
