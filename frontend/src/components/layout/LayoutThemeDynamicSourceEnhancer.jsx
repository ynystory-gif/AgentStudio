...
function install(panel){
  ...
  host.addEventListener('input',event=>{const t=event.target;if(!(t instanceof HTMLInputElement))return;const idx=t.dataset.urlIndex;if(idx!==undefined)state.urls[Number(idx)]=t.value})
  host.addEventListener('change',event=>{const t=event.target;if(t instanceof HTMLInputElement&&t.dataset.imageIndex!==undefined){state.files[Number(t.dataset.imageIndex)]=t.files?.[0]||null;render(host,state,status)}else if(t instanceof HTMLSelectElement&&t.dataset.roleIndex!==undefined){state.roles[Number(t.dataset.roleIndex)]=t.value}})
  ...
  action?.addEventListener('click',async event=>{
    ...
    try{
      ...
      const result=await api('/ui-themes/import-dynamic',{method:'POST',body:JSON.stringify({name,urls,images,scope:'GLOBAL'})})
      status.textContent=result?.message||'Theme 저장이 완료되었습니다.';status.className='ui-layout-dynamic-status ok'
      window.setTimeout(()=>window.location.reload(),700)
    }catch(error){status.textContent=String(error?.message||error);status.className='ui-layout-dynamic-status error'}finally{state.busy=false;if(action){action.disabled=false;action.textContent='분석 후 Theme 저장'}}
  },true)
}
