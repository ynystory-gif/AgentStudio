...
type CaseRow = LegacyRecord
type DatasetRow = LegacyRecord
type JobRow = LegacyRecord
...
export function LlmLearningCenter() {
  ...
  const [summary,setSummary]=useState<LegacyValue>({})
  ...
  const refresh=async(reason: LegacyValue='학습 데이터 로드')=>{
    ...
    const finishPart=(part:LearningLoadPart,label:string)=>{
      ...
      setLoadState((prev: LegacyValue)=>{
        const parts={...prev.parts,[part]:'done' as const}
        const completed=Object.values(parts).filter((value: LegacyValue)=>value==='done').length
    ...
    const summaryPromise=api<LegacyValue>('/learning/summary').then((value: LegacyValue)=>{
      ...
      return value
    }).catch((error: LegacyValue)=>{failPart('summary','학습 요약/모델 상태',error);throw error})
    const casesPromise=api<LegacyValue>(`/learning/misjudgments?limit=1000${provider?`&provider=${encodeURIComponent(provider)}`:''}`).then((value: LegacyValue)=>{
      if(requestId===refreshSequence.current)setCases(value?.items||[])
      finishPart('cases','오판 목록')
      return value
    }).catch((error: LegacyValue)=>{failPart('cases','오판 목록',error);throw error})
    const datasetsPromise=api<LegacyValue>('/learning/datasets').then((value: LegacyValue)=>{
      if(requestId===refreshSequence.current)setDatasets(value?.items||[])
      finishPart('datasets','Dataset/문제 목록')
      return value
    }).catch((error: LegacyValue)=>{failPart('datasets','Dataset/문제 목록',error);throw error})

    try{
      const [s,c,d]=await Promise.all([summaryPromise,casesPromise,datasetsPromise])
      if(requestId===refreshSequence.current){
        setLoadState((prev: LegacyValue)=>({...prev,visible:true,active:false,progress:100,completed:total,stage:'학습 데이터 로드 완료',elapsedSeconds:Math.max(0,Math.round((Date.now()-startedAt)/1000))}))
        window.setTimeout(()=>{
          if(requestId===refreshSequence.current)setLoadState((prev: LegacyValue)=>prev.active||prev.error?prev:{...prev,visible:false})
        },900)
      }
  ...
  useEffect(()=>{
    ...
    const timer=window.setInterval(()=>{
      setLoadState((prev: LegacyValue)=>prev.active?{...prev,elapsedSeconds:Math.max(0,Math.floor((Date.now()-prev.startedAt)/1000))}:prev)
    },500)
  ...
  useEffect(()=>{
    ...
    const timer=window.setInterval(async()=>{
      try{
        const next=await api<LegacyValue>(`/learning/problems/collect-job/${problemJob.id}`)
        setProblemJob(next)
        if(next?.status==='completed'){
          window.clearInterval(timer)
          const result=next?.result||{}
          const generated=Array.isArray(result?.datasets)?result.datasets:[]
          const generatedProblems=Number(result?.generated_problem_count||generated.reduce((sum:number,row: LegacyValue)=>sum+Number(row?.problem_count||row?.problems?.length||0),0))
          const requestedSourceIds=(result?.requested_source_case_ids||next?.source_case_ids||[]).map((value: LegacyValue)=>String(value||'')).filter(Boolean)
          const createdSourceIds=(result?.created_source_case_ids||generated.map((row: LegacyValue)=>row?.source_case_id)).map((value: LegacyValue)=>String(value||'')).filter(Boolean)
          ...
          if(generated.length){
            const generatedIds=new Set(generated.map((row: LegacyValue)=>String(row?.id||'')).filter(Boolean))
            const persisted=(refreshed?.datasets?.items||refreshed?.datasets||[]).find((row: LegacyValue)=>generatedIds.has(String(row?.id||''))) || generated[0]
  ...
  useEffect(()=>{
    ...
    const timer=window.setInterval(async()=>{
      try{
        const next=await api<LegacyValue>(`/learning/learning-apply-job/${applyJob.id}`)
        setApplyJob(next)
        if(next?.status==='completed'){
          window.clearInterval(timer)
  ...
  const providers=useMemo(()=>Array.from(new Set(cases.map((x: LegacyValue)=>String(x.provider||'unknown')))),[cases])
  const candidateCount=useMemo(()=>cases.filter((x: LegacyValue)=>x.status==='candidate').length,[cases])
  const confirmedCount=useMemo(()=>cases.filter((x: LegacyValue)=>x.status==='confirmed').length,[cases])
  const selectedDataset=useMemo(()=>datasets.find((x: LegacyValue)=>String(x.id)===selectedDatasetId)||null,[datasets,selectedDatasetId])

  const run=async(label:string,fn:()=>Promise<LegacyValue>)=>{
  ...
  const downloadSql=async(kind:LearningSqlKind)=>{
    try{
      const payload={
        kind,
        provider,
        case_ids:kind==='cases'?cases.map((row: LegacyValue)=>String(row?.id||'')).filter(Boolean):[],
        dataset_ids:kind==='cases'?[]:datasets.map((row: LegacyValue)=>String(row?.id||'')).filter(Boolean),
      }
      const item=await api<LegacyValue>('/learning/sql-export',{method:'POST',body:JSON.stringify(payload)})
  ...
  const startProblemCollection=async()=>{
    ...
    const eligibleCases=cases.filter((row: LegacyValue)=>String(row?.status||'').toLowerCase()==='confirmed'&&!row?.learning_exact_source_dataset_exists)
    const sourceCaseIds=eligibleCases.slice(0,maxCases).map((row: LegacyValue)=>String(row?.id||'')).filter(Boolean)
    ...
    try{
      const job=await api<LegacyValue>('/learning/problems/collect-job',{method:'POST',body:JSON.stringify({target_per_case:targetPerCase,max_cases:sourceCaseIds.length,provider:'ollama',source_case_ids:sourceCaseIds})})
      setProblemJob(job)
  ...
  const startLearningApply=async(row:DatasetRow)=>{
    ...
    try{setApplyJob(await api<LegacyValue>(`/learning/datasets/${row.id}/learning-apply-job`,{method:'POST'}))}
    catch(e){setMessage(String(e))}
  }
  ...
  const startDownloadRecommended=async()=>{setMessage('');try{setModelJob(await api<LegacyValue>('/learning/recommended-ollama/download-job',{method:'POST'}))}catch(e){setMessage(String(e))}}
  ...
  const allPcSummary=(row:DatasetRow)=>{
    ...
    return apps.map((x: LegacyValue)=>`${x.pc_name}: ${x.enabled?'사용 중':x.installed?'설치됨':'미적용'}${x.model_name?` (${x.model_name})`:''}`).join(' · ')
  }
  const fmtDate=(value: LegacyValue)=>{if(!value)return '-';const date=new Date(String(value));return Number.isNaN(date.getTime())?String(value):date.toLocaleString()}
