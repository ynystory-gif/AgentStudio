...
export function getEditorLanguage(filePath: LegacyValue = ''): string {
...
export function getEditorModelPath(projectRoot: LegacyValue = '', filePath: LegacyValue = ''): string {
...
export const isNotebookFile = (filePath: LegacyValue = ''): boolean => String(filePath || '').toLowerCase().endsWith('.ipynb')
export const isPdfFile = (filePath: LegacyValue = ''): boolean => String(filePath || '').toLowerCase().endsWith('.pdf')
export const isDatabaseDiagramFile = (filePath: LegacyValue = ''): boolean => String(filePath || '').toLowerCase().endsWith('.agentdiag.json')

export function isPresentationFile(filePath: LegacyValue = ''): boolean {
  const value = String(filePath || '').toLowerCase()
  return value.endsWith('.ppt') || value.endsWith('.pptx')
}

export function isImageFile(filePath: LegacyValue = ''): boolean {
  const ext = String(filePath || '').trim().toLowerCase().split('.').pop() || ''
  return ['png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp', 'ico', 'avif'].includes(ext)
}

export const isBinaryPreviewFile = (filePath: LegacyValue = ''): boolean =>
...
export function registerEscapedDoubleQuotePairGuard(editor: PairTypingEditorLike | null | undefined) {
  ...
  return editor.onKeyDown((event: LegacyValue) => {
