...
def _test_firestore(cfg: dict) -> dict:
    ...
    # Reading a non-existent document verifies auth/project/database without writing user data.
    client.collection("_agentstudio_connection_probe").document("__probe__").get()
    return {
        "ok": True,
        "provider": "firestore",
        "message": f"Firestore 연결 성공 · project={_text(cfg.get('project_id'))} · database={_text(cfg.get('database_id'), '(default)')}",
    }
...
def _redis_client(cfg: dict):
    ...
    return redis.Redis(
        ...
        ssl=_bool(cfg.get("tls", cfg.get("ssl"))),
...
def _test_redis(cfg: dict) -> dict:
    ...
    return {
        "ok": True,
        "provider": "redis",
        "message": f"Redis 연결 성공 · {_text(cfg.get('host'), '127.0.0.1')}:{int(cfg.get('port') or 6379)} / DB {int(cfg.get('db') or 0)}",
    }
